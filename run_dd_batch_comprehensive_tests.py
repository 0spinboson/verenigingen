    ...
    def run_single_test(self, test_class, verbose=False, coverage=False):
        ...
        if '.' in test_class:
            ...
            if 'TestDD' in test_class or 'TestSEPA' in test_class:
                ...
                test_file = f"verenigingen/tests/test_dd_batch_edge_cases_comprehensive.py::{module_parts[-1]}"
            else:
        ...
        security_checks = [
            {
                ...
                'test': lambda: __import__('verenigingen.utils.dd_security_enhancements', fromlist=['MemberIdentityValidator'])
            },
            {
                'name': 'Security Audit Logger Import', 
                'test': lambda: __import__('verenigingen.utils.dd_security_enhancements', fromlist=['DDSecurityAuditLogger'])
            },
            {
                'name': 'Conflict Resolution Manager Import',
                'test': lambda: __import__('verenigingen.utils.dd_security_enhancements', fromlist=['DDConflictResolutionManager'])
            }
        ]
    ...
    def generate_test_report(self, output_file=None):
        ...
        report = {
            ...
            'summary': {
                ...
                'success_rate': (self.results['passed'] / max(1, self.results['passed'] + self.results['failed'] + self.results['errors'])) * 100
            },
...
def main():
    ...
    parser.add_argument('command', choices=['all', 'smoke', 'security', 'edge_cases', 'performance', 'core', 'integration', 'benchmarks', 'validate'], 
