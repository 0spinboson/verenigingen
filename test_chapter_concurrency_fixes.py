...
def test_chapter_permission_optimization():
    ...
    try:
        ...
        start_time = time.time()
        conditions = frappe.get_method("verenigingen.verenigingen.doctype.chapter.chapter.get_chapter_permission_query_conditions")(current_user)
        conditions_time = time.time() - start_time
        
        print(f"✅ Optimized accessible chapters query: {optimized_time:.4f}s, Found: {len(accessible_chapters)}")
...
def test_chapter_performance_edge_cases():
    ...
    try:
        ...
        empty_chapters = frappe.get_all("Chapter", 
                                      fields=["name"], 
                                      limit=10)
        ...
        if test_chapter:
            ...
            print(f"Chair: {chair_member}, Head updated: {head_updated}, Board members: {len(board_members)}, Roles: {len(active_roles)}")
