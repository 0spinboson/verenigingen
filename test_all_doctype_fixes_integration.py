...
def test_member_volunteer_integration():
    ...
    try:
        ...
        print(f"✅ Volunteer operations: {volunteer_time:.4f}s, Assignments: {len(assignments)}, History: {len(history)}")
...
def test_chapter_member_volunteer_integration():
    ...
    try:
        ...
        print(f"✅ Chapter operations: {chapter_time:.4f}s, Chair: {chair_member}, Head updated: {head_updated}")
        print(f"✅ Board operations: {board_time:.4f}s, Members: {len(board_members)}, Is member: {is_board_member}, Role: {member_role}")
        print(f"✅ Volunteer integration: {volunteer_integration_time:.4f}s, Chapter assignments: {len(chapter_assignments)}")
...
def test_permission_system_integration():
    ...
    try:
        ...
        start_time = time.time()
        accessible_chapters = frappe.get_method("verenigingen.verenigingen.doctype.chapter.chapter.get_user_accessible_chapters_optimized")(current_user)
        chapter_conditions = frappe.get_method("verenigingen.verenigingen.doctype.chapter.chapter.get_chapter_permission_query_conditions")(current_user)
        ...
        if accessible_chapters:
            ...
            print(f"✅ Permission queries: {permission_time:.4f}s, Accessible chapters: {len(accessible_chapters)}")
            print(f"✅ Context loading: {context_time:.4f}s, Can view members: {context.get('is_board_member', False)}")
        else:
...
def test_performance_benchmarks():
    ...
    try:
        ...
        start_time = time.time()
        accessible_chapters = frappe.get_method("verenigingen.verenigingen.doctype.chapter.chapter.get_user_accessible_chapters_optimized")(frappe.session.user)
