...
class DDEnhancementValidator:
    ...
    def validate_javascript_syntax(self, file_path):
        ...
        try:
            ...
            return len(issues) == 0, issues, {
                'found_classes': found_classes,
                'js_class_count': len(found_classes)
            }
    ...
    def validate_file(self, file_path, file_type):
        ...
        if 'test_dd_batch_edge_cases' in file_name:
            ...
            if not structure_ok:
                ...
                print(f"      Classes: {details.get('class_count', 0)}, API functions: {details.get('api_count', 0)}")
    ...
    def run_validation(self):
        ...
        overall_success = (self.results['syntax_errors'] == 0 and 
                          success_count >= len([f for f in files_to_check if f['required']]))
