...
def debug_volunteer_lookup(user_email=None):
    ...
    member = frappe.db.get_value("Member", {"email": user_email}, ["name", "first_name", "last_name", "email"], as_dict=True)
    if member:
        ...
        volunteer_by_member = frappe.db.get_value("Volunteer", {"member": member.name}, ["name", "volunteer_name", "email", "member"], as_dict=True)
        if volunteer_by_member:
            print(f"Found Volunteer via Member: {volunteer_by_member.name} - {volunteer_by_member.volunteer_name}")
    ...
    volunteer_direct = frappe.db.get_value("Volunteer", {"email": user_email}, ["name", "volunteer_name", "email", "member"], as_dict=True)
    if volunteer_direct:
        print(f"Found Volunteer via direct email: {volunteer_direct.name} - {volunteer_direct.volunteer_name}")
    ...
    similar_members = frappe.db.sql("""
        ...
        WHERE email LIKE %s 
        ORDER BY email
    """, f"%{user_email.split('@')[0]}%", as_dict=True)
    ...
    if similar_volunteers:
        ...
        for volunteer in similar_volunteers[:5]:  # Limit to 5 results
            print(f"  {volunteer.name}: {volunteer.volunteer_name} - {volunteer.email} (member: {volunteer.member})")
