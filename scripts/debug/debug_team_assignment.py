...
def debug_team_assignment():
    ...
    try:
        ...
        for i, assignment in enumerate(volunteer.assignment_history or []):
            print(f"  {i+1}. {assignment.assignment_type}: {assignment.reference_doctype} {assignment.reference_name} - {assignment.role} ({assignment.status})")
    ...
    try:
        ...
        for member in team.team_members:
            print(f"  - {member.volunteer} ({member.volunteer_name}): {member.role} - Active: {member.is_active}")
