...
def debug_parko_issue():
    ...
    try:
        ...
        for i, bm in enumerate(board_members):
            print(f"  {i+1}. {bm.volunteer_name} ({bm.volunteer}) - Role: {bm.chapter_role} - Active: {bm.is_active}")
        ...
        # Search members
        members_found = frappe.get_all("Member", 
        ...
        if members_found:
            ...
            for member in members_found:
                print(f"  - {member.full_name} ({member.name}) - Status: {member.status} - Email: {member.email}")
        else:
            print("❌ No members found with Parko")
            
        # Also search for Janssen
        janssen_members = frappe.get_all("Member", 
        ...
        # Search volunteers
        volunteers_found = frappe.get_all("Volunteer", 
        ...
        if volunteers_found:
            ...
            for volunteer in volunteers_found:
                print(f"  - {volunteer.volunteer_name} ({volunteer.name}) - Member: {volunteer.member} - Status: {volunteer.status}")
        else:
