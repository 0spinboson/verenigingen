...
class TestExpenseSubmissionIntegration(unittest.TestCase):
    ...
    def test_complete_expense_submission_workflow(self):
        ...
        with patch('frappe.session.user', self.test_volunteer.email):
            ...
            self.assertEqual(volunteer_record.member, self.test_member.name, "Step 1: Member field should be correct")
        ...
        with patch('frappe.session.user', self.test_volunteer.email):
            ...
            self.assertTrue(result.get("success"), f"Step 2: Expense submission should succeed. Error: {result.get('message')}")
        ...
        if result.get("expense_claim_name"):
            ...
            self.assertEqual(flt(expense_claim.total_claimed_amount), flt(expense_data["amount"]), "Step 3: Amount should match")
        
        if result.get("expense_name"):
            volunteer_expense = frappe.get_doc("Volunteer Expense", result.get("expense_name"))
            self.assertEqual(volunteer_expense.volunteer, self.test_volunteer.name, "Step 3: Volunteer link should be correct")
            self.assertEqual(volunteer_expense.chapter, self.test_chapter.name, "Step 3: Chapter should be correct")
    ...
    def test_workflow_with_api_call_simulation(self):
        ...
        with patch('frappe.session.user', self.test_volunteer.email):
            ...
            self.assertTrue(result.get("success"), f"API simulation should succeed. Error: {result.get('message')}")
    ...
    def test_concurrent_submission_safety(self):
        ...
        for i, result in enumerate(results):
            ...
            self.assertIsNotNone(result.get("expense_claim_name"), f"Submission {i} should create expense claim")
    ...
    def test_data_consistency_across_records(self):
        ...
        with patch('frappe.session.user', self.test_volunteer.email):
            ...
            if result.get("expense_claim_name") and result.get("expense_name"):
                ...
                # Check linkage
                self.assertEqual(volunteer_expense.expense_claim_id, expense_claim.name, "Records should be linked")
                
                # Check amount consistency
                self.assertEqual(flt(volunteer_expense.amount), flt(expense_claim.total_claimed_amount), "Amounts should match")
                
                # Check date consistency
                self.assertEqual(str(volunteer_expense.expense_date), str(expense_claim.posting_date), "Dates should match")
