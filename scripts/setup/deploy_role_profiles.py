...
def deploy_enhanced_profiles():
    ...
    if not os.path.exists(enhanced_role_profile_path):
        print("❌ Enhanced role profile file not found. Please ensure role_profile_enhanced.json exists in fixtures/")
        return
        
    if not os.path.exists(enhanced_module_profile_path):
        print("❌ Enhanced module profile file not found. Please ensure module_profile_enhanced.json exists in fixtures/")
        return
...
def backup_existing_profiles():
    ...
    role_profiles = frappe.get_all("Role Profile", 
                                  filters={"name": ["like", "Verenigingen%"]},
                                  fields=["*"])
    ...
    if role_profiles:
        ...
        backup_path = os.path.join(frappe.get_app_path("verenigingen"), "fixtures", "role_profile_backup.json")
        with open(backup_path, 'w') as f:
            json.dump(backup_data, f, indent=2, default=str)
        print(f"  ✓ Backed up {len(backup_data)} role profiles to role_profile_backup.json")
    
    # Backup module profiles
    module_profiles = frappe.get_all("Module Profile",
                                    filters={"name": ["like", "Verenigingen%"]},
                                    fields=["*"])
    ...
    if module_profiles:
        ...
        backup_path = os.path.join(frappe.get_app_path("verenigingen"), "fixtures", "module_profile_backup.json")
        with open(backup_path, 'w') as f:
            json.dump(backup_data, f, indent=2, default=str)
