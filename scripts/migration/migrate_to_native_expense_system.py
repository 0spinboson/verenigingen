...
def update_employee_approvers():
    ...
    for volunteer_data in volunteers_with_employees:
        try:
            ...
            frappe.log_error(f"Error updating employee for volunteer {volunteer_data.name}: {str(e)}", 
...
def ensure_board_members_have_approver_roles():
    ...
    for member in board_members:
        try:
            ...
            frappe.log_error(f"Error updating roles for {member.volunteer}: {str(e)}", 
                           "Board Member Role Update Error")
...
def show_migration_summary():
    ...
    employees_with_approvers = frappe.db.count("Employee", 
        {"expense_approver": ["!=", ""]})
    
    # Count employees without approvers
    employees_without_approvers = frappe.db.count("Employee", 
...
def cleanup_department_hierarchy():
    ...
    dept_prefixes = ["National Organization", "National Board", "Chapters", 
                    "National Teams", "Chapter "]
    
    removed_count = 0
    for prefix in dept_prefixes:
        departments = frappe.get_all("Department",
            filters={"department_name": ["like", f"{prefix}%"]})
