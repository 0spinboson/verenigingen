...
def create_employee_for_volunteer(volunteer_name):
    ...
    try:
        ...
        employee_data = {
            ...
            "first_name": volunteer_doc.volunteer_name.split(' ')[0] if volunteer_doc.volunteer_name else "Volunteer",
            "company": company,
            "status": "Active",
        ...
        frappe.log_error(f"Manual employee creation failed for {volunteer_name}: {str(e)}", "Manual Employee Creation")
        return {
            "success": False,
...
def create_employees_for_all_volunteers():
    ...
    volunteers_without_employees = frappe.get_all(
        "Volunteer",
        filters={"employee_id": ["is", "not set"]},
...
def check_volunteer_employee_status():
    ...
    volunteers = frappe.get_all(
        "Volunteer",
        fields=["name", "volunteer_name", "employee_id", "personal_email"]
    )
