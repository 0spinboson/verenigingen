...
class PaymentMethodVerifier:
    ...
    def check_file(self, file_path: Path) -> None:
        ...
        for match in old_dd_matches:
            ...
            for pos in match_positions:
                ...
                if (not context_after.startswith(' Batch') and 
                    not context_before.endswith('SEPA') and
                    'Direct Debit Batch' not in content[max(0, pos-50):pos+50]):
        ...
        if filtered_dd_matches:
            ...
            if self.verbose:
                logger.warning(f"Found old 'Direct Debit' in {file_path}: {len(filtered_dd_matches)} instances")
        ...
        if double_sepa_matches:
            ...
            if self.verbose:
                logger.warning(f"Found double 'SEPA Direct Debit' in {file_path}: {len(double_sepa_matches)} instances")
    ...
    def generate_report(self) -> str:
        ...
        # Overall status
        issues_found = (self.results['old_direct_debit_count'] + 
