    ...
    for cmd in current_results:
        if cmd in baseline_data['results']:
            ...
            comparisons[cmd] = {
                ...
                'performance_change': 'improved' if time_diff < -0.5 else 'degraded' if time_diff > 0.5 else 'stable'
            }
            
            # Print comparison
            change_icon = "🚀" if time_diff < -0.5 else "🐌" if time_diff > 0.5 else "📊"
            print(f"{change_icon} {current_results[cmd]['description']}: {current_time:.2f}s (was {baseline_time:.2f}s, {time_diff:+.2f}s)")
...
def run_post_change_tests():
    ...
    result = subprocess.run([
        "python3", "regression_test_runner.py"
    ...
    if performance_comparison:
        performance_issues = [
            cmd for cmd, comparison in performance_comparison.items()
        ...
        if performance_issues:
            ...
            for cmd in performance_issues:
                ...
                print(f"   - {comp['current_time']:.2f}s vs {comp['baseline_time']:.2f}s ({comp['time_ratio']:.2f}x slower)")
...
def check_test_coverage():
    ...
    result = subprocess.run([
        "python3", "run_volunteer_portal_tests.py", "--coverage"
    ], capture_output=True, text=True)
