...
def test_volunteer_query_optimization():
    ...
    try:
        ...
        print(f"Optimized assignments: {len(assignments_optimized)}, Fallback assignments: {len(assignments_fallback)}")
        ...
        print(f"✅ Optimized history query: {optimized_history_time:.4f}s, Fallback history query: {fallback_history_time:.4f}s")
...
def test_volunteer_activity_management():
    ...
    try:
        ...
        result = volunteer_doc.end_activity(
            ...
            notes="Activity completed for testing"
        )
