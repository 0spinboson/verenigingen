...
class TestTeamAssignmentHistory(unittest.TestCase):
    ...
    def test_team_assignment_integration(self):
        ...
        if found_team_assignment:
            print(f"✅ Found team assignment: {found_team_assignment.assignment_type} - {found_team_assignment.role}")
            ...
            for assignment in volunteer_doc.assignment_history:
                print(f"  - {assignment.assignment_type}: {assignment.reference_doctype} {assignment.reference_name} ({assignment.status})")
    ...
    def test_team_member_field_configuration(self):
        ...
        expected_field_order = [
            "volunteer", "volunteer_name", "role_type", "role", "column_break_5",
            "from_date", "to_date", "is_active", "status", "notes"
        ]
        
        actual_field_order = [field.fieldname for field in team_member_meta.fields]
        self.assertEqual(actual_field_order, expected_field_order, "Team Member field order should match expected order")
        ...
        self.assertEqual(list_view_fields, expected_list_view, "Team Member list view fields should match expected fields")
    ...
    def test_volunteer_assignment_field_configuration(self):
        ...
        expected_field_order = [
            "reference_doctype", "reference_name", "assignment_type", "role", "column_break_4",
            "start_date", "end_date", "status", "hours_section", "estimated_hours", "actual_hours",
            "details_section", "accomplishments", "notes"
        ]
        
        actual_field_order = [field.fieldname for field in assignment_meta.fields]
        self.assertEqual(actual_field_order, expected_field_order, "Volunteer Assignment field order should match expected order")
        ...
        expected_list_view = ["reference_doctype", "reference_name", "assignment_type", "role", "start_date", "end_date"]
        self.assertEqual(list_view_fields, expected_list_view, "Volunteer Assignment list view fields should match expected fields")
        ...
        try:
            ...
            for assignment in volunteer_doc.assignment_history or []:
                if (assignment.reference_name and 
                    (assignment.reference_name.startswith("TEST") or 
                     assignment.reference_name.startswith("Assignment Test") or
                     assignment.reference_name.startswith("Test Team"))):
                    assignments_to_remove.append(assignment)
