...
class TestMemberManagementAPI(VereningingenUnitTestCase):
    ...
    def test_update_member_information(self):
        ...
        result = member_management.update_member_information(member.name, update_data)
    ...
    def test_remove_member_from_chapter(self):
        ...
        result = member_management.remove_member_from_chapter(
            member.name,
            chapter.name,
            reason="Moved to different region"
        )
        
        # Verify removal
        chapter_doc = frappe.get_doc("Chapter", chapter.name)
        active_members = [cm for cm in chapter_doc.chapter_members 
                         if cm.member == member.name and cm.status == "Active"]
    ...
    def test_update_member_status(self):
        ...
        # Suspend member
        result = member_management.update_member_status(
            member.name,
            "Suspended",
            reason="Non-payment"
        )
    ...
    def test_bulk_member_update(self):
        ...
        if hasattr(member_management, 'bulk_update_members'):
            ...
            result = member_management.bulk_update_members(
                member_names,
                update_data
            )
    ...
    def test_member_communication_preferences(self):
        ...
        if hasattr(member_management, 'update_communication_preferences'):
            ...
            result = member_management.update_communication_preferences(
                member.name,
                preferences
            )
