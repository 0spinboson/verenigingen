...
class TestPaymentProcessingAPI(VereningingenUnitTestCase):
    ...
    def test_process_membership_payment(self):
        ...
        result = payment_processing.process_membership_payment(
            member.name,
            payment_data
        )
    ...
    def test_create_sepa_mandate(self):
        ...
        result = payment_processing.create_sepa_mandate(
            member.name,
            mandate_data
        )
    ...
    def test_process_direct_debit_batch(self):
        ...
        batch_data = {
            ...
            "member_payments": [
                {
                    "member": mp["member"].name,
                    "amount": mp["amount"]
                }
                for mp in members_with_payments
            ]
        }
    ...
    def test_handle_payment_failure(self):
        ...
        result = payment_processing.handle_payment_failure(
            member.name,
            failure_data
        )
    ...
    def test_retry_failed_payment(self):
        ...
        retry_data = {
            "amount": 100.00,
            "retry_method": "SEPA Direct Debit"
        }
        
        result = payment_processing.retry_failed_payment(
            member.name,
            retry_data
        )
    ...
    def test_generate_payment_reminder(self):
        ...
        if hasattr(payment_processing, 'generate_payment_reminder'):
            ...
            result = payment_processing.generate_payment_reminder(
                member.name,
                reminder_data
            )
            ...
            self.assertEqual(result["status"], "success")
            self.assertions.assert_email_sent(
                member.email,
    ...
    def test_payment_method_update(self):
        ...
        result = payment_processing.update_payment_method(
            member.name,
            update_data
        )
