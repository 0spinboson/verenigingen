...
class TestPolicyExpenseIntegration(unittest.TestCase):
    ...
    def test_policy_expense_submission_allowed_for_any_volunteer(self):
        ...
        with patch('verenigingen.templates.pages.volunteer.expenses.is_policy_covered_expense', return_value=True):
            with patch('verenigingen.templates.pages.volunteer.expenses.get_user_volunteer_record', return_value=mock_volunteer):
                with patch('frappe.get_doc', return_value=MagicMock(name="EXP-POLICY-001")):
                    with patch('verenigingen.templates.pages.volunteer.expenses.get_or_create_expense_type', return_value="Travel"):
                        with patch('verenigingen.templates.pages.volunteer.expenses.get_organization_cost_center', return_value="National Cost Center"):
    ...
    def test_non_policy_expense_requires_board_membership(self):
        ...
        with patch('verenigingen.templates.pages.volunteer.expenses.get_user_volunteer_record', return_value=mock_volunteer):
            with patch('frappe.get_single') as mock_get_single:
                ...
                # Mock no board membership (regular volunteer)
                with patch('frappe.db.exists', return_value=False):
                    with patch('verenigingen.templates.pages.volunteer.expenses.is_policy_covered_expense', return_value=False):
    ...
    def test_policy_expense_with_board_member(self):
        ...
        with patch('verenigingen.templates.pages.volunteer.expenses.get_user_volunteer_record', return_value=mock_volunteer):
            with patch('frappe.get_doc', return_value=MagicMock(name="EXP-BOARD-001")):
                with patch('verenigingen.templates.pages.volunteer.expenses.get_or_create_expense_type', return_value="Equipment"):
                    with patch('verenigingen.templates.pages.volunteer.expenses.get_organization_cost_center', return_value="National Cost Center"):
                        with patch('frappe.get_single') as mock_get_single:
                            # Mock settings with national chapter
                            mock_settings = MagicMock()
    ...
    def test_policy_expense_categories_setup(self):
        ...
        with patch('frappe.get_all', return_value=mock_categories):
            ...
            policy_categories = frappe.get_all("Expense Category", 
...
class TestPolicyExpenseReporting(unittest.TestCase):
    ...
    def test_policy_expense_tracking_in_report(self):
        ...
        from verenigingen.verenigingen.report.chapter_expense_report.chapter_expense_report import build_expense_row
    ...
    def test_policy_expense_statistics(self):
        ...
        test_expenses = [
            ...
            {'amount': 75, 'status': 'Approved', 'category_name': 'Materials', 'organization_type': 'National'},
            {'amount': 500, 'status': 'Approved', 'category_name': 'Equipment', 'organization_type': 'National'},
        ...
        policy_expenses = [exp for exp in test_expenses 
                          if exp['organization_type'] == 'National' and 
                             exp['category_name'] in ['Travel', 'Materials']]
