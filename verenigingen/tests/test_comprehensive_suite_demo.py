...
class TestComprehensiveSuiteDemo(VereningingenWorkflowTestCase, PersonaTestMixin):
    ...
    def test_state_management_demo(self):
        ...
        self.assertTrue(
            state_manager.validate_transition("DemoEntity", "demo1", "Initial", "Processing")
        )
        self.assertTrue(
            state_manager.validate_transition("DemoEntity", "demo1", "Processing", "Completed")
        )
    ...
    def test_cleanup_management_demo(self):
        ...
        cleanup_manager.register("Membership", "test-membership-1", 
                                dependencies=["Member:test-member-1"])
        cleanup_manager.register("Volunteer", "test-volunteer-1",
                                dependencies=["Member:test-member-1"])
    ...
    def test_concurrent_workflow_demo(self):
        ...
        workflow_classes = [
            TestMemberLifecycle,
            TestVolunteerJourney,
            TestPaymentFailureRecovery
        ]
        
        for workflow_class in workflow_classes:
            self.assertTrue(hasattr(workflow_class, "test_complete_member_lifecycle") or
                          hasattr(workflow_class, "test_complete_volunteer_journey") or
                          hasattr(workflow_class, "test_payment_failure_recovery_workflow"))
...
class TestSuiteHealthCheck(VereningingenWorkflowTestCase):
    ...
    def test_workflow_classes_health(self):
        ...
        for workflow_class in workflow_classes:
            ...
            test_methods = [method for method in dir(workflow_class) 
