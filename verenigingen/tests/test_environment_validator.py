...
class TestEnvironmentValidator:
    ...
    def validate_frappe_environment(self):
        ...
        try:
            ...
            if not site_name:
                return {
                    'status': 'fail',
                    'message': 'No site configured',
                    'errors': ['Site name not found']
                }
    ...
    def validate_database_connection(self):
        ...
        try:
            ...
            db_size = frappe.db.sql("SELECT ROUND(SUM(data_length + index_length) / 1024 / 1024, 1) as db_size_mb FROM information_schema.tables WHERE table_schema = DATABASE()", as_dict=True)
    ...
    def validate_required_doctypes(self):
        ...
        try:
            ...
            return {
                'status': 'fail',
                'message': f'Doctype validation failed: {str(e)}',
                'errors': [str(e)]
            }
        ...
        try:
            ...
            for pattern in test_patterns:
                chapters = frappe.get_all("Chapter", 
                    filters=[["chapter_name", "like", f"%{pattern}%"]],
                    limit=5)
                existing_test_records.extend(chapters)
            
            if len(existing_test_records) > 50:
                warnings.append(f"Many existing test records found ({len(existing_test_records)}), consider cleanup")
            ...
            return {
                'status': 'fail',
                'message': f'Test data validation failed: {str(e)}',
                'errors': [str(e)]
            }
    ...
    def validate_network_connectivity(self):
        ...
        try:
            ...
            result = {
                'status': status,
                'message': message,
                'details': {'tests_run': len(connectivity_tests)}
            }
