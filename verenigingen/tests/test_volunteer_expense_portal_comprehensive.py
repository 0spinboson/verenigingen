...
class TestVolunteerExpensePortal(unittest.TestCase):
	...
	def test_expense_naming_pattern(self):
		...
		self.assertTrue(re.match(pattern, expense_name), 
						f"Expense name '{expense_name}' doesn't match expected pattern")
...
class TestVolunteerExpenseNaming(unittest.TestCase):
	...
	def test_naming_series_format(self):
		...
		from verenigingen.verenigingen.doctype.volunteer_expense.volunteer_expense import VolunteerExpense
