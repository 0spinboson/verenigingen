...
class TestSEPAMandateCreation(unittest.TestCase):
    ...
    def test_get_active_sepa_mandate_with_iban_filter(self):
        ...
        try:
            ...
            result = get_active_sepa_mandate(
                member=self.member.name,
                iban="DE89370400440532013000"
            )
    ...
    def test_create_and_link_mandate_enhanced_basic(self):
        ...
        mandate_links = [link for link in member_doc.sepa_mandates 
                        if link.mandate_reference == "ENHANCED-MANDATE-001"]
    ...
    def test_create_and_link_mandate_enhanced_replace_existing(self):
        ...
        try:
            ...
            member_doc.reload()
            old_links = [link for link in member_doc.sepa_mandates 
                        if link.mandate_reference == "OLD-MANDATE-001"]
            self.assertEqual(len(old_links), 1)
            self.assertFalse(old_links[0].is_current)
            
            # Verify new mandate link is current
            new_links = [link for link in member_doc.sepa_mandates 
                        if link.mandate_reference == "NEW-MANDATE-001"]
