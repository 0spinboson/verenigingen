...
class TestFinancialIntegrationEdgeCases(unittest.TestCase):
    ...
    def test_membership_fee_precision(self):
        ...
        for input_amount, expected in test_amounts:
            ...
            self.assertEqual(
                f"{membership.annual_fee:.2f}", 
                expected,
                f"Amount {input_amount} not rounded correctly"
            )
    ...
    def test_payment_amount_mismatch(self):
        ...
        with patch('verenigingen.api.financial.process_payment') as mock_payment:
            ...
            with self.assertRaises((frappe.ValidationError, ValueError)):
                ...
                frappe.call("verenigingen.api.financial.validate_payment", 
                          membership=membership.name, amount=50.00)
    ...
    def test_orphaned_subscription_cleanup(self):
        ...
        member_backup = self.member.name
        self.member.delete()
    ...
    def test_fee_change_audit_trail(self):
        ...
        try:
            audit_entries = frappe.get_all("Member Fee Change History", 
