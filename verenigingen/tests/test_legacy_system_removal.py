...
class TestLegacySystemRemoval(unittest.TestCase):
    ...
    def test_chapter_expense_report_uses_erpnext_only(self):
        ...
        from verenigingen.verenigingen.report.chapter_expense_report.chapter_expense_report import get_erpnext_expense_data
    ...
    def test_workspace_links_updated(self):
        ...
        try:
            ...
            if 'db_links' in result:
                expense_links = [link for link in result['db_links'] if 'expense' in link.get('label', '').lower()]
    ...
    def test_volunteer_expense_portal_still_functional(self):
        ...
        with patch('frappe.session.user', 'test@example.com'):
            with patch('verenigingen.templates.pages.volunteer.expenses.get_user_volunteer_record') as mock_get_volunteer:
                ...
                with patch('verenigingen.templates.pages.volunteer.expenses.get_volunteer_organizations', return_value={"chapters": [], "teams": []}):
                    with patch('verenigingen.templates.pages.volunteer.expenses.get_expense_categories', return_value=[]):
                        with patch('verenigingen.templates.pages.volunteer.expenses.get_volunteer_expenses', return_value=[]):
                            with patch('verenigingen.templates.pages.volunteer.expenses.get_expense_statistics', return_value={}):
                                with patch('verenigingen.templates.pages.volunteer.expenses.get_approval_thresholds', return_value={}):
                                    with patch('verenigingen.templates.pages.volunteer.expenses.get_national_chapter', return_value=None):
            ...
            if os.path.exists(file_path):
                with open(file_path, 'r') as f:
                    ...
                    for legacy_import in legacy_imports:
                        self.assertNotIn(legacy_import, content, 
    ...
    def test_workspace_configuration_complete(self):
        ...
        try:
            ...
            if os.path.exists(workspace_file):
                ...
                erpnext_links = [l for l in links if 'erpnext' in l.get('label', '').lower()]
...
class TestERPNextMigrationCompliance(unittest.TestCase):
    ...
    def test_expense_claim_field_compatibility(self):
        ...
        required_fields = [
            ...
            'status', 'approval_status', 'employee', 'employee_name', 'remark', 'company'
        ]
            ...
            with patch('frappe.db.sql') as mock_sql:
                ...
                if mock_sql.called:
                    for call in mock_sql.call_args_list:
                        ...
                        self.assertNotIn('tabVolunteer Expense', query, "Should not query legacy Volunteer Expense table")
                        self.assertNotIn('tabExpense Approval Dashboard', query, "Should not query legacy dashboard table")
    ...
    def test_employee_integration_functional(self):
        ...
        with patch('verenigingen.templates.pages.volunteer.expenses.get_user_volunteer_record', return_value=mock_volunteer):
            with patch('frappe.get_doc', return_value=MagicMock(name="EXP-TEST")):
                with patch('verenigingen.templates.pages.volunteer.expenses.get_or_create_expense_type', return_value="Travel"):
                    with patch('verenigingen.templates.pages.volunteer.expenses.get_organization_cost_center', return_value="Test Center"):
                        with patch('frappe.defaults.get_global_default', return_value="Test Company"):
                            with patch('frappe.db.get_value', return_value="Test Account"):
