...
class TestVolunteerPortalEdgeCases(FrappeTestCase):
    ...
    def setup_test_data(cls):
        ...
        for chapter_name, enabled in [
            (cls.active_chapter, 1),
            (cls.disabled_chapter, 0)
        ]:
        ...
        for team_name, status in [
            (cls.active_team, "Active"),
            (cls.inactive_team, "Inactive")
        ]:
    ...
    def setup_chapter_memberships(cls):
        ...
        if not member_exists:
            active_chapter_doc.append("members", {
                "member": cls.active_member,
        ...
        if not member_exists:
            disabled_chapter_doc.append("members", {
                "member": cls.disabled_member,
                "chapter_join_date": today(),
    ...
    def setup_team_memberships(cls):
        ...
        if not frappe.db.exists("Team Member", {"volunteer": cls.active_volunteer, "parent": cls.active_team}):
        ...
        if not frappe.db.exists("Team Member", {"volunteer": cls.disabled_volunteer, "parent": cls.inactive_team}):
            team_member = frappe.get_doc({
                "doctype": "Team Member",
    ...
    def setup_expense_categories(cls):
        ...
        for category_name, disabled in [
            (cls.active_category, 0),
            (cls.disabled_category, 1)
        ]:
            if not frappe.db.exists("Expense Category", category_name):
                category = frappe.get_doc({
                    "doctype": "Expense Category",
                    "category_name": category_name,
                    "description": f"Edge test category",
    ...
    def test_volunteer_with_multiple_chapter_memberships(self):
        ...
        extra_chapter_doc.append("members", {
    ...
    def test_volunteer_member_with_expired_chapter_membership(self):
        ...
        expired_chapter_doc.append("members", {
            "member": self.active_member,
