...
class TestDDMemberIdentityEdgeCases(FrappeTestCase):
    ...
    def test_identical_names_different_addresses(self):
        ...
        john1 = self.create_test_member(
            "John", "Smith", 
            f"john.smith.amsterdam.{self.unique_id}@test.com",
            "NL43INGB0123456789"
        )
        
        john2 = self.create_test_member(
            "John", "Smith",
            f"john.smith.rotterdam.{self.unique_id}@test.com", 
            "NL43ABNA0987654321"
        )
        ...
        self.assertTrue(len(results["potential_duplicates"]) >= 2, 
        ...
        for duplicate in results["potential_duplicates"]:
            self.assertGreater(duplicate["name_similarity"], 0.9,
                             "Name similarity should be very high for identical names")
    ...
    def test_similar_names_fuzzy_matching(self):
        ...
        self.assertGreater(len(results["potential_duplicates"]), 0,
                          "Should detect some similar names")
        
        # Check that fuzzy matching works
        found_johns = [d for d in results["potential_duplicates"] 
                      if "john" in d["existing_name"].lower()]
        self.assertGreater(len(found_johns), 0, 
                          "Should find existing Johns through fuzzy matching")
    ...
    def test_shared_family_bank_account(self):
        ...
        husband = self.create_test_member(
            "John", "Doe", 
            f"john.doe.{self.unique_id}@test.com",
            shared_iban
        )
        
        wife = self.create_test_member(
            "Jane", "Doe",
            f"jane.doe.{self.unique_id}@test.com", 
            shared_iban
        )
        ...
        self.assertTrue(results.get("valid", False), 
                       "Should allow family account sharing")
        
        analysis = results.get("sharing_analysis", {})
        self.assertEqual(analysis.get("pattern"), "family_account",
                        "Should identify as family account pattern")
        self.assertLess(analysis.get("risk_level", 1.0), 0.5,
                       "Risk level should be low for family accounts")
    ...
    def test_corporate_shared_accounts(self):
        ...
        self.assertFalse(results.get("valid", True), 
                        "Should block unrelated members using same account")
        
        analysis = results.get("sharing_analysis", {})
        self.assertEqual(analysis.get("pattern"), "suspicious_sharing",
                        "Should identify as suspicious sharing pattern")
        self.assertGreater(analysis.get("risk_level", 0.0), 0.7,
                          "Risk level should be high for unrelated sharing")
    ...
    def test_payment_amount_anomalies(self):
        ...
        self.assertAny(["Zero or negative amount" in reason for reason in anomaly_reasons],
                      "Should detect zero/negative amounts")
        self.assertAny(["Unusually high amount" in reason for reason in anomaly_reasons], 
    ...
    def test_multiple_payments_same_iban(self):
        ...
        self.assertGreater(len(suspicious), 0, 
                          "Should detect multiple payments from same IBAN as suspicious")
        
        # Should flag high payment count
        pattern = suspicious[0] if suspicious else {}
        self.assertGreater(pattern.get("risk_level", 0.0), 0.3,
                          "Should have elevated risk level for multiple payments")
        self.assertIn("Many payments from same account", 
                     pattern.get("issues", []),
...
class TestDDBatchSecurityValidation(FrappeTestCase):
        ...
        for malicious_iban in malicious_ibans:
            try:
                ...
                self.assertIsInstance(results, dict, 
                                    f"Should handle malicious IBAN gracefully: {malicious_iban[:20]}...")
                
                # Should not crash or return sensitive data
                if not results.get("valid", False):
                    self.assertIn("error", results, 
                                "Invalid IBAN should return proper error message")
    ...
    def test_malicious_data_injection_names(self):
        ...
        for malicious_name in malicious_names:
            ...
            try:
                ...
                self.assertIsInstance(results, dict,
                                    f"Should handle malicious name gracefully: {malicious_name[:20]}...")
    ...
    def test_security_event_escalation(self):
        ...
        security_events = frappe.get_all(
            ...
            filters={
                "severity": "high",
                "details": ["like", f"%{self.unique_id}%"]
            },
            fields=["name", "event_type", "severity", "description"]
        )
        
        self.assertEqual(len(security_events), len(high_severity_events),
                        "All high severity events should be logged")
...
class TestDDConflictResolution(FrappeTestCase):
    ...
    def test_conflict_report_creation(self):
        ...
        if report_id:
            ...
            self.assertIn("high-risk", report.summary.lower(), 
                         "Summary should mention high-risk matches")
    ...
    def test_automatic_conflict_resolution(self):
        ...
        self.assertGreater(len(results["resolved"]), 0, 
                          "Should auto-resolve low-risk conflicts")
        
        # High risk should require manual review
        self.assertGreater(len(results["unresolved"]), 0,
                          "Should leave high-risk conflicts for manual review")
        
        self.assertTrue(results["requires_manual_review"],
                       "Should flag that manual review is required")
    ...
    def test_resolution_rule_customization(self):
        ...
        strict_results = self.conflict_manager.auto_resolve_conflicts(
            test_conflicts, strict_rules
        )
        
        # Should require manual review with strict rules
        self.assertEqual(len(strict_results["resolved"]), 0,
                        "Strict rules should not auto-resolve medium-risk conflicts")
        ...
        lenient_results = self.conflict_manager.auto_resolve_conflicts(
            test_conflicts, lenient_rules
        )
        
        # Should auto-resolve with lenient rules
        self.assertGreater(len(lenient_results["resolved"]), 0,
                          "Lenient rules should auto-resolve medium-risk conflicts")
...
class TestDDPerformanceEdgeCases(FrappeTestCase):
    ...
    def test_large_member_database_performance(self):
        ...
        self.assertLess(creation_time, 30.0, 
                       f"Creating {member_count} members should take less than 30 seconds")
        ...
        self.assertAny([name for name in matching_names if "TestMember25" in name],
                      "Should find the matching member")
    ...
    def test_large_batch_anomaly_detection(self):
        ...
        self.assertAny(["Zero or negative amount" in reason for reason in warning_reasons],
                      "Should detect zero amount anomaly")
        self.assertAny(["Unusually high amount" in reason for reason in warning_reasons],
