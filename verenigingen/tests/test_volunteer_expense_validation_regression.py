...
class TestVolunteerExpenseValidationRegression(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        chapter_member_name = f"{cls.test_chapter.name}-{cls.test_member.name}"
        if not frappe.db.exists("Chapter Member", {"parent": cls.test_chapter.name, "member": cls.test_member.name}):
    ...
    def test_get_user_volunteer_record_includes_member_field(self):
        ...
        with patch('frappe.session.user', self.test_volunteer.email):
            ...
            self.assertIsNotNone(volunteer_record, "get_user_volunteer_record should return a volunteer record")
            self.assertTrue(hasattr(volunteer_record, 'member'), "Volunteer record must include 'member' field")
            self.assertIsNotNone(volunteer_record.member, "Member field should not be None")
            self.assertEqual(volunteer_record.member, self.test_member.name, "Member field should match expected member")
            ...
            self.assertTrue(hasattr(volunteer_record, 'volunteer_name'), "Volunteer record must include 'volunteer_name' field")
    ...
    def test_get_user_volunteer_record_via_member_lookup(self):
        ...
        with patch('frappe.session.user', self.test_member.email):
            ...
            self.assertTrue(hasattr(volunteer_record, 'member'), "Member field must be included in member-based lookup")
    ...
    def test_chapter_membership_validation_with_valid_member(self):
        ...
        with patch('frappe.session.user', self.test_volunteer.email):
            ...
            self.assertIn("successfully", result.get("message", "").lower(), "Success message should indicate success")
    ...
    def test_chapter_membership_validation_without_member_field_fails(self):
        ...
        with patch('verenigingen.templates.pages.volunteer.expenses.get_user_volunteer_record', return_value=mock_volunteer_without_member):
            with patch('frappe.session.user', self.test_volunteer.email):
                result = submit_expense(expense_data)
                
                # Should fail because volunteer record lacks member field
                self.assertFalse(result.get("success"), "Expense submission should fail when member field is missing")
    ...
    def test_volunteer_record_field_completeness(self):
        ...
        with patch('frappe.session.user', self.test_volunteer.email):
            ...
            for field in required_fields:
                self.assertTrue(hasattr(volunteer_record, field), f"Volunteer record must include '{field}' field")
                
                if field == 'member':
                    # Member field should not be None for volunteers with member links
                    self.assertIsNotNone(getattr(volunteer_record, field), f"'{field}' field should not be None for linked volunteers")
    ...
    def test_chapter_membership_query_correctness(self):
        ...
        correct_result = frappe.db.exists("Chapter Member", {
        ...
        incorrect_result = frappe.db.exists("Chapter Member", {
            "parent": self.test_chapter.name,
            "volunteer": self.test_volunteer.name  # This field doesn't exist in Chapter Member
        })
        self.assertFalse(incorrect_result, "Incorrect query using 'volunteer' field should not find membership")
    ...
    def test_expense_submission_flow_integration(self):
        ...
        with patch('frappe.session.user', self.test_volunteer.email):
            ...
            self.assertTrue(result.get("success"), f"Integration test should succeed. Error: {result.get('message')}")
    ...
    def tearDownClass(cls):
        ...
        try:
            ...
            frappe.db.delete("Expense Claim", {"employee": cls.test_volunteer.employee_id}) if hasattr(cls.test_volunteer, 'employee_id') else None
            frappe.db.delete("Volunteer Expense", {"volunteer": cls.test_volunteer.name})
            frappe.db.delete("Chapter Member", {"parent": cls.test_chapter.name, "member": cls.test_member.name})
