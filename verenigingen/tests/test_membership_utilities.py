...
class MembershipTestUtilities:
    ...
    @staticmethod
    def create_membership_type_with_subscription(name, period="Monthly", amount=100.0, 
    ...
    @staticmethod
    def create_membership_with_subscription(member, membership_type, start_date=None, 
                                          submit=True, custom_amount=None):
        """
    ...
    @staticmethod
    def create_membership_via_application(member, membership_type, approver_email=None, 
                                        custom_amount=None):
        """
        ...
        try:
            ...
            result = membership_application_review.approve_membership_application(
                ...
                membership_type=membership_type.name if isinstance(membership_type, frappe.model.document.Document) else membership_type,
                custom_amount=custom_amount
            )
    ...
    def cleanup_test_membership_types(prefix="Test"):
        ...
        for mt in test_types:
            ...
            if mt.subscription_plan:
                frappe.delete_doc("Subscription Plan", mt.subscription_plan, 
                                ignore_permissions=True, force=True)
                
            # Delete linked items
            items = frappe.get_all(
                "Item",
                filters={"item_name": ["like", f"{mt.name}%"]},
                fields=["name"]
            )
            for item in items:
            ...
            frappe.delete_doc("Membership Type", mt.name, 
                            ignore_permissions=True, force=True)
