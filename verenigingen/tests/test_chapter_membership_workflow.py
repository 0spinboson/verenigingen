...
class TestChapterMembershipWorkflow(unittest.TestCase):
    ...
    def test_create_member_with_chapter_selection(self):
        ...
        from verenigingen.utils.application_helpers import create_member_from_application, determine_chapter_from_application
        ...
        self.assertEqual(suggested_chapter, self.test_chapter, "Chapter should be determined from application data")
        ...
        self.assertEqual(member.application_status, "Pending", "Member should have pending application status")
    ...
    def test_create_pending_chapter_membership(self):
        ...
        pending_records = frappe.get_all("Chapter Member", 
    ...
    def test_activate_pending_chapter_membership(self):
        ...
        active_records = frappe.get_all("Chapter Member",
    ...
    def test_member_approval_activates_chapter_membership(self):
        ...
        with patch.object(member, 'create_membership_on_approval', return_value=True):
            ...
            result = member.approve_application()
            ...
            with patch('frappe.session.user', 'regular@user.com'):
                with patch('frappe.db.get_value', return_value=None):  # No member/volunteer found
                    try:
                        ...
                        test_member_row = next((row for row in data if row.get("member") == member.name), None)
                        self.assertIsNone(test_member_row, "Pending member should not be visible to regular users")
    ...
    def test_duplicate_pending_membership_prevention(self):
        ...
        second_result = create_pending_chapter_membership(member, self.test_chapter)
        # Should either return existing record or None (not create duplicate)
        
        # Verify only one record exists
        pending_count = frappe.db.count("Chapter Member", 
            filters={"member": member.name, "parent": self.test_chapter})
    ...
    def test_chapter_member_status_filtering(self):
        ...
        pending_members = frappe.get_all("Chapter Member",
        ...
        active_members = frappe.get_all("Chapter Member",
