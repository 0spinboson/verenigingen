...
class TestVolunteerJourney(VereningingenWorkflowTestCase):
    ...
    def _stage_2_complete_profile(self, context):
        ...
        with self.as_user(user_email):
            ...
            if not volunteer.volunteer_interests:
                volunteer.append("volunteer_interests", {
                    "interest_category": "Events",
                    "interest_level": "High"
                })
                volunteer.append("volunteer_interests", {
                    "interest_category": "Youth Programs",
    ...
    def _stage_7_generate_reports(self, context):
        ...
        total_hours = sum(frappe.get_doc("Volunteer Activity", activity["activity_name"]).hours 
                         for activity in activities_logged)
        total_expenses = sum(frappe.get_doc("Volunteer Expense", expense["expense_name"]).amount 
    ...
    def _stage_8_deactivate_volunteer(self, context):
        ...
        with self.as_user(user_email):
            ...
            teams = frappe.get_all("Team", 
                                 filters={"team_members.volunteer": volunteer_name},
                                 fields=["name"])
    ...
    def _validate_volunteer_deactivated(self, context):
        ...
        for team_info in teams:
            team = frappe.get_doc("Team", team_info.name)
            active_memberships = [tm for tm in team.team_members 
                                if tm.volunteer == volunteer_name and tm.is_active]
            self.assertEqual(len(active_memberships), 0, 
                           "All team memberships should be deactivated")
    ...
    def _validate_complete_volunteer_journey(self):
        ...
        if volunteer_name:
            ...
            activities = frappe.get_all("Volunteer Activity", 
            ...
            expected_states = ["Created", "Profile Completed", "Teams Joined", 
                             "Expenses Submitted", "Hours Tracked", "Deactivated"]
