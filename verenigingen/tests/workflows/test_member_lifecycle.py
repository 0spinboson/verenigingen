...
class TestMemberLifecycle(VereningingenWorkflowTestCase):
    ...
    def _stage_2_review_approve(self, context):
        ...
        with self.as_user(self.admin_user.name):
            ...
            selected_chapter = context.get("application_data", {}).get("chapter_preference") or self.test_chapter.name
            if selected_chapter:
        ...
        with self.as_user(self.admin_user.name):
            ...
            if hasattr(member, 'application_invoice'):
                ...
                payment_result = process_application_payment(
                    member_name,
                    "SEPA Direct Debit",
                    payment_reference="TEST-LIFECYCLE-001"
                )
            else:
    ...
    def _stage_7_member_activities(self, context):
        ...
        return {
            "team_name": team.name,
            "expense_name": expense.name if expense else None
        }
    ...
    def _validate_complete_lifecycle(self):
        ...
        if member_name:
            ...
            self.assertEqual(final_member_state, "Terminated", 
                         "Member should be in terminated state")