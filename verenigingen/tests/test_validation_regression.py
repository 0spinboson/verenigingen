...
class TestValidationRegression(unittest.TestCase):
                ...
                for valid_value in validation_data["valid"]:
                    with self.subTest(doctype=doctype_name, field=field_name, value=valid_value):
                        try:
                            ...
                            self.fail(f"Valid value '{valid_value}' for {doctype_name}.{field_name} should not cause error: {str(e)}")
    ...
    def test_application_flow_volunteer_status_regression(self):
        ...
        if result.get("success"):
            ...
            volunteers = frappe.get_all(
                "Volunteer",
                filters={"member": member_record},
                fields=["name", "status"]
            )
            
            if volunteers:
                self.add_cleanup("Volunteer", volunteers[0]["name"])
                self.assertEqual(volunteers[0]["status"], "New", 
                ...
                try:
                    ...
                    for field in status_fields:
                        if field.fieldtype == "Select":
                            ...
                            self.assertTrue(field.options and field.options.strip(), 
    ...
    def test_helper_function_validations(self):
        ...
        if volunteer:
            ...
            self.assertIn(volunteer.status, ["New", "Onboarding", "Active", "Inactive", "Retired"],
                         "create_volunteer_record should use valid status values")
            self.assertTrue(isinstance(volunteer.available, int) and volunteer.available in [0, 1],
...
def run_validation_regression_suite():
    ...
    try:
        ...
        summary["message"] = (f"Validation regression tests: {summary['tests_run']} run, "
