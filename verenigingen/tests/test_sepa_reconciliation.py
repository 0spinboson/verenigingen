...
class TestSEPAReconciliation(unittest.TestCase):
        ...
        if not frappe.db.exists('Bank Account', 'TEST-RECON-BANK'):
            cls.test_bank_account = frappe.get_doc({
                ...
                'account': frappe.db.get_value('Account', 
                    {'account_type': 'Bank', 'is_group': 0}, 'name')
            }).insert()
        else:
    ...
    def setUp(self):
        ...
        self.test_batch.append('invoices', {
    ...
    def test_match_by_batch_reference(self):
        ...
        transaction = self.create_test_transaction(
            100, 
            f'SEPA DD {self.test_batch.name} TEST-RECON-DESC'
        )
    ...
    def test_match_by_description_patterns(self):
        ...
        transaction = self.create_test_transaction(
            100,
            f'Payment for invoice {self.test_invoice.name}',
            'NL91ABNA0417164300'  # Different IBAN
        )
        ...
        invoice_matches = [m for m in matches if m['type'] == 'invoice' 
                          and m['reference'] == self.test_invoice.name]
        self.assertTrue(len(invoice_matches) > 0)
    ...
    def test_fuzzy_name_matching(self):
        ...
        for name1, name2, min_score in test_cases:
            ...
            self.assertGreaterEqual(score, min_score - 0.1, 
                f"Score for '{name1}' vs '{name2}' too low: {score}")
    ...
    def test_auto_create_payment_entry(self):
        ...
        transaction = self.create_test_transaction(
            100,
            f'SEPA DD {self.test_batch.name}'
        )
        ...
        payment_entry = self.reconciliation_engine.create_payment_entry(
            transaction, 
            best_match
        )
    ...
    def test_partial_payment_handling(self):
        ...
        transaction = self.create_test_transaction(
            50,  # Half the invoice amount
            f'Partial payment for {self.test_invoice.name}'
        )
    ...
    def test_duplicate_transaction_handling(self):
        ...
        transaction1 = self.create_test_transaction(
            100,
            f'SEPA DD {self.test_batch.name} DUPLICATE-TEST'
        )
        
        matches = self.reconciliation_engine.match_transaction(transaction1)
        payment1 = self.reconciliation_engine.create_payment_entry(
            transaction1, 
            matches[0]
        )
        payment1.submit()
        ...
        transaction2 = self.create_test_transaction(
            100,
            f'SEPA DD {self.test_batch.name} DUPLICATE-TEST'
        )
        ...
        with self.assertRaises(Exception):
            ...
            if matches2:
                ...
                payment2 = self.reconciliation_engine.create_payment_entry(
                    transaction2,
                    matches2[0]
                )
    ...
    def test_reconcile_bank_transactions(self):
        ...
        # Good match
        transactions.append(self.create_test_transaction(
            100,
            f'SEPA DD {self.test_batch.name}'
        ))
    ...
    def test_match_threshold_configuration(self):
        ...
        transaction = self.create_test_transaction(
            100,
            'Vague description',
            'NL99UNKNOWN9999999'  # Unknown IBAN
        )
