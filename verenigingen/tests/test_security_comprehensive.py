    ...
    def test_cross_organization_data_leakage(self):
        ...
        for member in members:
            if member.name in [self.member1.name, self.member2.name]:
                self.assertEqual(member.chapter, self.chapter1.name, 
                    "Chapter admin can see members from other chapters")
        ...
        self.assertTrue(len(accessible_volunteers) <= 1,
            "Chapter admin can access volunteers from other chapters")
