            ...
            for status in valid_options:
                with self.subTest(status=status):
                    ...
                    try:
                        ...
                        self.fail(f"Valid member status '{status}' should not cause validation error: {str(e)}")
        ...
        for app_status in valid_app_statuses:
            with self.subTest(app_status=app_status):
                ...
                try:
                    ...
                    self.fail(f"Valid application status '{app_status}' should not cause validation error: {str(e)}")
        ...
        try:
            ...
            if result.get("success"):
                ...
                if volunteer_records:
                    self.assertEqual(len(volunteer_records), 1, "Exactly one volunteer record should be created")
                    self.assertEqual(volunteer_records[0]["status"], "New", "Volunteer should have 'New' status")
