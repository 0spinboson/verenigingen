...
class ContributionAmendmentRequest(Document):
        ...
        if membership.subscription:
            ...
            try:
                if hasattr(subscription, 'billing_interval_count'):
                    self.current_billing_interval = f"{subscription.billing_interval_count} {subscription.billing_interval}(s)"
    ...
    def before_insert(self):
        ...
        if (self.amendment_type == "Fee Change" and 
            self.requested_amount and self.current_amount and 
            self.requested_amount > self.current_amount):
    ...
    def apply_amendment(self):
        ...
        if getdate(self.effective_date) > getdate(today()):
            ...
            frappe.msgprint(
                _("This amendment is scheduled to be applied automatically on {0}. You cannot apply it manually before the effective date.").format(effective_date_formatted),
        ...
        if old_subscription:
            try:
                ...
                linked_invoices = frappe.get_all("Sales Invoice", 
                    filters={"subscription": old_subscription, "docstatus": 1})
...
def create_fee_change_amendment(member_name, new_amount, reason, effective_date=None):
    ...
    if (amendment.current_amount and new_amount > amendment.current_amount and 
