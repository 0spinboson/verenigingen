...
frappe.ui.form.on('Membership Type', {
	refresh: function (frm) {
		frappe.db.get_single_value('Verenigingen Settings', 'enable_razorpay_for_memberships').then(val => {
			if (val) frm.set_df_property('razorpay_plan_id', 'hidden', false);
		});

		frappe.db.get_single_value('Verenigingen Settings', 'allow_invoicing').then(val => {
