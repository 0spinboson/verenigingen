...
class TestRegion(FrappeTestCase):
    ...
    def setUpClass(cls):
        ...
        test_regions = frappe.get_all("Region", filters={
            "region_name": ["like", "Test Region%"]
        })
        ...
        test_members = frappe.get_all("Member", filters={
            "email": ["like", "test%coordinator%"]
        })
    ...
    def tearDown(self):
        ...
        test_regions = frappe.get_all("Region", filters={
            "region_name": ["like", f"%{self.test_suffix}%"]
        })
        for region in test_regions:
            try:
                frappe.delete_doc("Region", region.name, force=True)
    ...
    def test_02_region_code_validation(self):
        ...
        region2 = self.create_test_region(
            custom_name=f"Invalid Region {self.test_suffix}",
            custom_code="123-ABC"
        )
        ...
        region3 = self.create_test_region(
            custom_name=f"Too Long Region {self.test_suffix}",
            custom_code="TOOLONG"
        )
        with self.assertRaises(frappe.ValidationError):
            region3.save()
        ...
        region4 = self.create_test_region(
            custom_name=f"Too Short Region {self.test_suffix}",
            custom_code="T"
        )
        with self.assertRaises(frappe.ValidationError):
            region4.save()
    ...
    def test_03_region_uniqueness(self):
        ...
        region2 = self.create_test_region(
            custom_code=f"DUP{self.test_suffix[:2]}"
        )
    ...
    def test_04_postal_code_pattern_matching(self):
        ...
        region = self.create_test_region(
            postal_code_patterns="2000-2999, 3*, 4100, 5000-5099"
        )
        region.save()
    ...
    def test_05_postal_code_edge_cases(self):
        ...
        region = self.create_test_region(
            postal_code_patterns="1*, 2000-2999"
        )
        region.save()
    ...
    def test_06_coordinator_validation(self):
        ...
        region = self.create_test_region(
            regional_coordinator=member.name  # Use member name, not email
        )
        region.save()
    ...
    def test_10_postal_code_pattern_parsing(self):
        ...
        region = self.create_test_region(
            postal_code_patterns="1000-1999, 2*, 3000, 4000-4099"
        )
        region.save()
