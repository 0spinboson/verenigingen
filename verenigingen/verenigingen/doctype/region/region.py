...
class Region(WebsiteGenerator):
    ...
    def validate_region_code(self):
        ...
        if self.region_code:
            ...
            # Check uniqueness
            existing = frappe.db.get_value("Region", 
                {"region_code": self.region_code, "name": ["!=", self.name]})
            if existing:
            ...
            try:
                ...
                for pattern in patterns:
                    ...
                    if not re.match(r'^[\d\*\-\s]+$', pattern.strip()):
                        frappe.msgprint(_("Postal code pattern '{0}' may not be valid").format(pattern), 
                                      indicator="orange")
            except Exception:
    ...
    def get_region_statistics(self):
        ...
        try:
            ...
            stats['published_chapters'] = frappe.db.count("Chapter", 
                {"region": self.name, "published": 1})
...
def get_regional_coordinator(region_name):
    ...
    return frappe.db.get_value("Region", region_name, 
        ["regional_coordinator", "backup_coordinator"], as_dict=True)
