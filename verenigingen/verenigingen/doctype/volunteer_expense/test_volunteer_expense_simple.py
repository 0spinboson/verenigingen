...
class TestVolunteerExpenseSimple(unittest.TestCase):
	def test_basic_doctype_creation(self):
		...
		expected_required_fields = ["volunteer", "expense_date", "category", "description", "amount", "organization_type", "company"]
		
		for field in expected_required_fields:
			self.assertIn(field, [f.fieldname for f in meta.fields], f"Field {field} should exist in Volunteer Expense")
	...
	def test_expense_category_doctype_creation(self):
		...
		for field in expected_required_fields:
			self.assertIn(field, [f.fieldname for f in meta.fields], f"Field {field} should exist in Expense Category")