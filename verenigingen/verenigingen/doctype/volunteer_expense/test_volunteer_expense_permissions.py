...
class TestVolunteerExpensePermissions(unittest.TestCase):
	...
	def setup_test_data(cls):
		...
		if not frappe.db.exists("Chapter", "Permission Test Chapter"):
			cls.test_chapter = frappe.get_doc({
				"doctype": "Chapter",
				"chapter_name": "Permission Test Chapter",
				"chapter_code": "PTC001"
			})
		...
		if not frappe.db.exists("Account", f"Permission Test Expenses - {cls.company[:2]}"):
			cls.expense_account = frappe.get_doc({
				...
				"parent_account": frappe.get_value("Account", {
					...
					"company": cls.company
				}, "name") or "Application of Funds (Assets) - TC"
			})
	...
	@classmethod
	def create_test_user(cls, email, full_name, roles, organization=None, is_board_member=False, is_team_lead=False):
		...
		if organization:
			...
			if organization.doctype == "Chapter":
				...
				if is_board_member and not frappe.db.exists("Chapter Board Member", {
				...
				if not frappe.db.exists("Team Member", {
					"volunteer": volunteer.name,
	...
	def test_can_approve_expense_chapter_board_member(self):
		...
		expense = self.create_test_expense(
			self.chapter_volunteer_user["volunteer"].name,
			"Chapter",
	...
	def test_can_approve_expense_team_lead(self):
		...
		# Create test expense
		expense = self.create_test_expense(
			self.team_volunteer_user["volunteer"].name,
			"Team",
			self.test_team.name
		)
