...
class TestVolunteerExpenseMinimal(unittest.TestCase):
	...
	def setup_test_volunteer(cls):
		...
		if cls.test_chapter and not frappe.db.exists("Chapter Member", {
			"member": cls.test_member.name,
	...
	def test_approval_functions_exist(self):
		...
		from verenigingen.verenigingen.doctype.volunteer_expense.volunteer_expense import approve_expense, reject_expense, can_approve_expense
	...
	def tearDownClass(cls):
		...
		try:
			...
			if hasattr(cls, 'test_chapter') and cls.test_chapter:
				chapter_members = frappe.get_all("Chapter Member", filters={
					"member": cls.test_member.name,
					"chapter": cls.test_chapter
				})
