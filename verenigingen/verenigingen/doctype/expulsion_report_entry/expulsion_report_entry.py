...
class ExpulsionReportEntry(Document):
    ...
    def notify_governance_team(self):
        ...
        try:
            ...
            frappe.log_error(f"Failed to send expulsion notification: {str(e)}", "Expulsion Notification Error")
    ...
    def send_reversal_notification(self):
        ...
        try:
            ...
            frappe.log_error(f"Failed to send reversal notification: {str(e)}", "Expulsion Reversal Notification Error")
...
def generate_expulsion_governance_report(date_range=None, chapter=None):
    ...
    overdue_appeals = frappe.db.count("Termination Appeals Process", {
    ...
    if overdue_appeals > 0:
        compliance_issues.append({
            "issue": "Overdue Appeal Reviews",
            "count": overdue_appeals,
            "severity": "Critical"
        })
...
def get_member_expulsion_history(member_id):
    ...
    history = frappe.get_all(
        ...
        fields=[
            "name", "expulsion_date", "expulsion_type", "status", 
            "under_appeal", "reversal_date", "reversal_reason",
            "initiated_by", "approved_by", "chapter_involved"
        ],
