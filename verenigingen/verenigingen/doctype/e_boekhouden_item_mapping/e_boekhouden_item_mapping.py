    ...
    def validate(self):
        ...
        if self.account_code:
            account = frappe.db.get_value("Account", {
                "account_number": self.account_code,
        ...
        existing = frappe.db.exists("E-Boekhouden Item Mapping", {
        ...
        if existing:
            frappe.throw(f"A mapping already exists for account {self.account_code} and transaction type {self.transaction_type}")
...
def get_item_for_account(account_code, company, transaction_type="Both"):
    ...
    mapping = frappe.db.get_value("E-Boekhouden Item Mapping", {
...
def create_default_mappings(company):
    ...
    for account in accounts:
        ...
        if frappe.db.exists("E-Boekhouden Item Mapping", {
        ...
        if matched_item:
            ...
            mapping.transaction_type = "Sales" if account.root_type == "Income" else "Purchase" if account.root_type == "Expense" else "Both"
            mapping.is_active = 1
