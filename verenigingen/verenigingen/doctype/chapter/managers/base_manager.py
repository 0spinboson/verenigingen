...
class BaseManager(ABC):
    ...
    def log_action(self, action: str, details: Dict = None, level: str = "info"):
        ...
        if level == "error":
            frappe.log_error(
                ...
                title=f"Chapter Manager Error: {action}"
            )
        elif level == "warning":
    ...
    def send_notification(self, template: str, recipients: List[str], 
                         context: Dict, subject: str = None):
        """
        ...
        try:
            ...
            if not frappe.db.exists("Email Template", template):
                self.log_action(
                    f"Email template '{template}' not found",
            ...
            self.log_action(
                f"Notification sent via template '{template}'",
                {"recipients": len(recipients)}
            )
    ...
    def create_comment(self, comment_type: str, content: str, 
    ...
    def validate_permissions(self, action: str, user: str = None) -> bool:
        ...
        if "Association Manager" in frappe.get_roles(user):
            return True
    ...
    def _is_user_board_member(self, user: str) -> Optional[Dict]:
        ...
        for board_member in self.chapter_doc.board_members or []:
            if (board_member.volunteer in volunteer_ids and 
