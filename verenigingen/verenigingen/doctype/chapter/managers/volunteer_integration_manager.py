...
class VolunteerIntegrationManager(BaseManager):
    ...
    def add_volunteer_assignment_history(self, volunteer_id: str, role: str, 
                                       start_date: str, assignment_type: str = "Board Position") -> bool:
        """
    ...
    def sync_board_members_with_volunteer_system(self) -> Dict:
        ...
        try:
            ...
            for board_member in self.chapter_doc.board_members or []:
                try:
                    ...
                    if not board_member.volunteer:
                        sync_stats["warnings"].append(f"Board member missing volunteer ID")
                        continue
                    ...
                    sync_stats["errors"].append(f"Error processing volunteer {board_member.volunteer}: {str(e)}")
            
            self.log_action(
                "Volunteer system sync completed",
                sync_stats
            )
            ...
            self.log_action(
                "Critical error in volunteer system sync",
                {"error": str(e)},
                "error"
            )
            return {
    ...
    def get_volunteer_assignment_history(self, volunteer_id: str) -> List[Dict]:
        ...
        try:
            ...
            for assignment in volunteer.assignment_history or []:
                if (assignment.reference_doctype == "Chapter" and 
                    assignment.reference_name == self.chapter_name):
    ...
    def get_chapter_volunteer_statistics(self) -> Dict:
        ...
        try:
            ...
            self.log_action(
                "Error calculating volunteer statistics",
                {"error": str(e)},
                "error"
            )
            return {"error": str(e)}
    ...
    def validate_volunteer_board_consistency(self) -> Dict:
        ...
        try:
                ...
                if board_member.is_active:
                    ...
                    if board_member.to_date:
                        matching_assignments = [
                            ...
                            if (a["role"] == board_member.chapter_role and 
                                getdate(a.get("start_date")) == getdate(board_member.from_date))
                        ]
            ...
            self.log_action(
                "Error validating volunteer board consistency",
                {"error": str(e)},
                "error"
            )
            return {
    ...
    def migrate_volunteer_assignments(self) -> Dict:
        ...
        try:
            ...
            self.log_action(
                "Volunteer assignment migration completed",
                migration_stats
            )
            ...
            self.log_action(
                "Critical error in volunteer assignment migration",
                {"error": str(e)},
                "error"
            )
            return {
    ...
    def cleanup_orphaned_assignments(self) -> Dict:
        ...
        try:
            ...
            """, (self.chapter_name,), as_dict=True)
            
            # Get current board member volunteer IDs
            current_volunteers = {
                bm.volunteer for bm in self.chapter_doc.board_members or []
                if bm.volunteer
            }
            ...
            self.log_action(
                "Orphaned assignment cleanup completed",
                cleanup_stats
            )
            ...
            self.log_action(
                "Critical error in orphaned assignment cleanup",
                {"error": str(e)},
                "error"
            )
            return {
    ...
    def get_summary(self) -> Dict:
        ...
        try:
            ...
            self.log_action(
                "Error generating volunteer integration summary",
                {"error": str(e)},
                "error"
            )
            return {
