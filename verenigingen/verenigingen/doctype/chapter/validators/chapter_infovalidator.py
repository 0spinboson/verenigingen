...
class ChapterInfoValidator(BaseValidator):
    ...
    def validaterequired_fields(self, chapter_data: Dict, result: ValidationResult):
    ...
    def validatefield_formats(self, chapter_data: Dict, result: ValidationResult):
        """Validate field formats"""
    ...
    def validatefield_lengths(self, chapter_data: Dict, result: ValidationResult):
    ...
    def validatebusiness_rules(self, chapter_data: Dict, result: ValidationResult):
    ...
    def validateroute_format(self, route: str, result: ValidationResult):
        """Validate route format"""
        # Route should not start or end with slash
        if route.startswith('/') or route.endswith('/'):
            result.addwarning(
                ("Route should not start or end with forward slash")
            )

        # Route should only contain valid URL characters
        if not re.match(r'^[a-zA-Z0-9\-_/]+$', route):
            result.adderror(
                ("Route contains invalid characters")
            )

        # Check if route is already used
        if self.chapter_doc and frappe.db.exists("Chapter", {"route": route, "name": ["!=", self.chapter_doc.name]}):
            result.adderror(
                ("Route '{0}' is already used by another chapter").format(route)
            )

    def validatehtml_content(self, html_content: str, result: ValidationResult):
        """Validate HTML content for security and format"""
        # Basic HTML validation
        if '<script' in html_content.lower():
            result.adderror(
                ("Script tags are not allowed in HTML content for security reasons")
            )
        ...
        for attr in dangerous_attrs:
            if attr in html_content.lower():
                result.addwarning(
                    ("HTML content contains potentially unsafe attribute: {0}").format(attr)
                )

        # Validate HTML structure (basic check)
        self._validate_html_structure(html_content, result)

    def validatehtml_structure(self, html_content: str, result: ValidationResult):
        ...
        if unbalanced_tags:
            result.add_warning(
                ("HTML appears to have unbalanced tags: {0}").format(', '.join(unbalancedtags))
            )

    def validateunique_name(self, chapter_data: Dict, result: ValidationResult):
    ...
    def validateregion(self, region: str, result: ValidationResult):
        ...
        if valid_regions and region not in valid_regions:
            result.addwarning(
                ("Region '{0}' is not in the standard regions list").format(region)
            )

    def validatechapter_head(self, chapter_data: Dict, result: ValidationResult):
        """Validate chapter head assignment"""
        ...
        if not frappe.db.exists("Member", chapter_head):
            result.add_error(
                ("Chapter head member '{0}' does not exist").format(chapterhead)
            )
            return
        ...
        if member_status != "Active":
            result.add_warning(
                ("Chapter head member is not active (status: {0})").format(memberstatus)
            )

        # Optional: Check if member is part of the chapter
        # This might be implemented based on your business logic

    def validatepublished_status(self, chapter_data: Dict, result: ValidationResult):
        """Validate published status"""
        if chapter_data.get('published'):
            ...
            if missing_fields:
                result.adderror(
                    ("Cannot publish chapter. Missing required fields: {0}").format(
                        ', '.join(missing_fields)
                    )
                )
            ...
            if intro_length < 50:
                result.add_warning(
                    ("Chapter introduction is very short ({0} characters). Consider adding more detail before publishing").format(introlength)
                )
    ...
    def validate_chapter_route(self, route: str, chapter_name: str = None) -> ValidationResult:
        ...
        if route_parts[0] in system_routes:
            result.add_error(
                ("Route conflicts with system route: {0}").format(routeparts[0])
            )
    ...
    def validate_address_format(self, address: str) -> ValidationResult:
        ...
        if len(address.strip()) < 10:
            result.addwarning(
                ("Address appears to be very short")
            )
        ...
        if not has_number:
            result.addwarning(
                ("Address doesn't appear to contain a street number")
            )

        return result

    def getvalid_regions(self) -> Optional[list]:
