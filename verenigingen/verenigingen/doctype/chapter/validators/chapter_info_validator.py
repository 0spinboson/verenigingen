...
class ChapterInfoValidator(BaseValidator):
    ...
    def _validate_chapter_head(self, chapter_data: Dict, result: ValidationResult):
        ...
        if not frappe.db.exists("Member", chapter_head):
            result.add_error(
                ("Chapter head member '{0}' does not exist").format(chapter_head)
            )
            return
        ...
        if member_status != "Active":
            result.add_warning(
                ("Chapter head member is not active (status: {0})").format(member_status)
            )
    ...
    def _validate_published_status(self, chapter_data: Dict, result: ValidationResult):
        ...
        if chapter_data.get('published'):
            ...
            if intro_length < 50:
                result.add_warning(
                    ("Chapter introduction is very short ({0} characters). Consider adding more detail before publishing").format(intro_length)
                )
    ...
    def validate_chapter_route(self, route: str, chapter_name: str = None) -> ValidationResult:
        ...
        if route_parts[0] in system_routes:
            result.add_error(
                ("Route conflicts with system route: {0}").format(route_parts[0])
            )
