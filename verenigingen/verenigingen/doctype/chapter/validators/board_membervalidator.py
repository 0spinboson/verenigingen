...
class BoardMemberValidator(BaseValidator):
    ...
    def validate_role_uniqueness(self, role: str, current_member_id: str, 
        ...
        for member in active_members:
            if (member.get('chapter_role') == role and 
    ...
    def validate_board_member_changes(self, old_members: List[Dict], 
                                    new_members: List[Dict]) -> ValidationResult:
    ...
    def validatemember_change(self, old_member: Dict, new_member: Dict) -> ValidationResult:
        ...
        if (old_member.get('chapter_role') != new_member.get('chapter_role') and 
            new_member.get('is_active')):
    ...
    def get_validation_summary(self, board_members: List[Dict]) -> Dict:
        ...
        return {
            ...
            'unique_roles_assigned': len([m for m in active_members 
                                        if m.get('chapter_role') in unique_roles]),
