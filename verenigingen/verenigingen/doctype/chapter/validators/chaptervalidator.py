...
class ChapterValidator(BaseValidator):
    ...
    def validate_board_member_change(self, old_board_members: List[Dict], 
                                   new_board_members: List[Dict]) -> ValidationResult:
    ...
    def validate_role_assignment(self, role: str, member_id: str, 
                               active_members: List[Dict]) -> ValidationResult:
                ...
                try:
                    ...
                    if volunteer_doc.member:
                        member_in_chapter = any(
                            m.get('member') == volunteer_doc.member and m.get('enabled')
                            for m in members
                        )
    ...
    def chapterto_dict(self, chapter_doc) -> Dict:
        ...
        # Basic fields
        fields = ['name', 'region', 'introduction', 'address', 'route', 'published', 
                 'chapter_head', 'postal_codes', 'meetup_embed_html']
    ...
    def boardmembers_to_list(self, board_members) -> List[Dict]:
        ...
        for member in board_members:
            member_dict = {}
            fields = ['name', 'volunteer', 'volunteer_name', 'email', 'chapter_role', 
                     'from_date', 'to_date', 'is_active', 'notes']
    ...
    def get_validation_summary(self) -> Dict:
        ...
        if not self.chapter_doc:
            return {
                ...
                'message': 'No chapter document provided'
            }
    ...
    def checkpublication_readiness(self, chapter_data: Dict) -> Dict:
        ...
        has_chair = any(
            self._is_chair_role(m.get('chapter_role')) 
            for m in active_board 
