...
def check_sepa_mandate_discrepancies():
    ...
    try:
        ...
        for member_data in members_with_direct_debit:
            try:
                ...
                results["errors"].append({
                    ...
                    "error": str(e),
                    "action": "member_processing_failed"
                })
        ...
        frappe.logger().info(f"Results: {results['total_checked']} checked, "
...
def _update_mandate_account_name(mandate_name, new_account_name):
    ...
    frappe.logger().info(f"Auto-updated SEPA mandate {mandate.mandate_id} account name from '{old_name}' to '{new_account_name}'")
