...
class MemberContactRequest(Document):
	...
	def create_crm_lead(self):
		...
		try:
			...
			frappe.log_error(f"Failed to create CRM Lead for Member Contact Request {self.name}: {str(e)}", "CRM Integration Error")
	...
	def notify_staff(self):
		...
		try:
			...
			frappe.log_error(f"Failed to send notification for Member Contact Request {self.name}: {str(e)}", "Notification Error")
...
@frappe.whitelist()
def create_contact_request(member, subject, message, request_type="General Inquiry", 
                          preferred_contact_method="Email", urgency="Normal", preferred_time=None):
...
def get_member_contact_requests(member, limit=10):
	...
	contact_requests = frappe.get_all("Member Contact Request",
