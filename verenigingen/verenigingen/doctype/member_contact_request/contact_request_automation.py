...
def send_follow_up_reminders():
    ...
    for request in overdue_requests:
        try:
            ...
            frappe.log_error(f"Error sending follow-up reminder for {request.name}: {str(e)}", "Follow-up Reminder Error")
    ...
    for urgency, threshold_days in urgency_thresholds.items():
        ...
        for request in overdue_requests:
            try:
                ...
                frappe.log_error(f"Error escalating contact request {request.name}: {str(e)}", "Escalation Error")
...
def auto_close_resolved_requests():
    ...
    for request in resolved_requests:
        try:
            ...
            frappe.log_error(f"Error auto-closing contact request {request.name}: {str(e)}", "Auto-close Error")
        ...
        try:
            if frappe.db.exists("Lead", request.crm_lead):
                ...
                if lead_doc.status != new_lead_status:
                    ...
                    frappe.logger().info(f"Synced status for CRM Lead {request.crm_lead} to {new_lead_status}")
                    
        except Exception as e:
            frappe.log_error(f"Error syncing CRM status for contact request {request.name}: {str(e)}", "CRM Sync Error")
...
def create_opportunity_from_contact_request(contact_request_name):
    ...
    try:
        ...
        frappe.log_error(f"Error creating opportunity from contact request {contact_request_name}: {str(e)}", "Opportunity Creation Error")
...
def get_contact_request_analytics():
    ...
    try:
        ...
        return {
            ...
            "avg_response_time_days": avg_response_time[0].avg_days if avg_response_time and avg_response_time[0].avg_days else 0,
