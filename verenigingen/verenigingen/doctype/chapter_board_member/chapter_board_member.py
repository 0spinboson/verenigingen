...
class ChapterBoardMember(Document):
    ...
    def remove_board_member_role(self):
        ...
        active_board_positions = frappe.db.count("Chapter Board Member", {
        ...
        if total_active_positions == 0:
            ...
            role_assignment = frappe.db.exists("Has Role", {
                "parent": user,
                "role": "Chapter Board Member"
            })
    ...
    def validate(self):
        ...
        if self.volunteer:
            ...
            if volunteer_doc.member:
                user = frappe.db.get_value("Member", volunteer_doc.member, "user")
                if not user:
                    frappe.msgprint(f"Warning: Volunteer {self.volunteer} does not have a linked user account. Board member role cannot be assigned.", 
                                  indicator="orange")
