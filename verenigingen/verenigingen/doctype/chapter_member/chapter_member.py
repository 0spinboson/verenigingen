...
class ChapterMember(Document):
	...
	def validate_chapter_membership_tracking(self):
		...
		is_allowed_context = any(
			context in "".join(call_stack) for context in allowed_contexts
		)
		
		# Also allow if the change is being made by administrator or system user
		current_user = frappe.session.user
		is_admin_user = current_user in ["Administrator", "Guest"] or "System Manager" in frappe.get_roles(current_user)
		
		# Allow if we're updating an existing record without changing key fields
		if not self.is_new() and not self.has_value_changed("member") and not self.has_value_changed("parent"):
			return
		...
		if not (is_allowed_context or is_admin_user):
			...
			frappe.msgprint(
				_("Chapter membership changes should be made through the Chapter Management interface or using the ChapterMembershipManager utility for proper history tracking."),
