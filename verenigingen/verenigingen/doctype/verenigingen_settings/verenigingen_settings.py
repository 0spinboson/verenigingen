	...
	def validate_donation_accounts(self):
		...
		if self.automate_donation_payment_entries:
			if not self.donation_payment_account:
				frappe.throw(_('Donation Payment Account is required when automating donation payment entries'))
		...
		if self.campaign_donation_account and self.campaign_donation_account == self.donation_payment_account:
			frappe.msgprint(_('Campaign Donation Account should be different from main Donation Payment Account for proper fund segregation'), indicator='yellow')
		
		if self.restricted_donation_account and self.restricted_donation_account == self.donation_payment_account:
			frappe.msgprint(_('Restricted Donation Account should be different from main Donation Payment Account for proper fund segregation'), indicator='yellow')
...
def validate_donation_configuration():
    ...
    validation_results["configuration"]["automate_donation_payment_entries"] = settings.automate_donation_payment_entries
    ...
    if not getattr(settings, 'campaign_donation_account', None):
        validation_results["warnings"].append("Campaign Donation Account not configured - campaign earmarking will not create separate GL entries")
    
    if not getattr(settings, 'restricted_donation_account', None):
        validation_results["warnings"].append("Restricted Donation Account not configured - specific goal earmarking will not create separate GL entries")
    ...
    if not donation_types:
        validation_results["warnings"].append("No Donation Types configured - create some donation types for better categorization")
    
    # Check Donor Type exists
    if not getattr(settings, 'default_donor_type', None):
        donor_types = frappe.get_all("Donor Type", limit=1)
        if donor_types:
            validation_results["warnings"].append("Default Donor Type not set - new donors will need manual type assignment")
        else:
            validation_results["errors"].append("No Donor Types exist - create donor types before accepting donations")
