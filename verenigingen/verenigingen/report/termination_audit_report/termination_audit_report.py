...
from frappe.utils import getdate, today, add_days, date_diff, flt
import json
...
def get_columns():
    return [
        ...
        {
            "fieldname": "audit_entries",
            "label": _("Audit Entries"),
            "fieldtype": "Int",
            "width": 100
        },
        {
            "fieldname": "system_updates",
...
def get_data(filters):
    ...
    for row in results:
        ...
        audit_count = frappe.db.count("Termination Audit Entry", {
            "parent": row.request_id,
            "parenttype": "Membership Termination Request"
        })
...
def get_summary(data, filters):
    ...
    non_compliant = len([r for r in data if r["compliance_status"] in ["Non-Compliant", "Critical"]])
    ...
    most_active_requester = max(requester_counts.items(), key=lambda x: x[1])[0] if requester_counts else "N/A"
    ...
    summary = [
        ...
        },
        {
            ...
            "indicator": "green" if avg_processing_time <= 7 else "orange" if avg_processing_time <= 14 else "red"
        },
        {
            "label": _("Executed"),
            "value": status_counts.get("Executed", 0),
            "indicator": "gray"
        },
        {
            "label": _("Pending"),
        ...
        {
            "label": _("Most Active Requester"),
            "value": most_active_requester,
            "indicator": "blue"
        }
    ]
...
def export_audit_report(filters=None):
    ...
    return {
        "columns": columns,
        "data": export_data,
        "file_name": f"termination_audit_report_{today()}.xlsx"
    }
