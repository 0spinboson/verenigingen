        ...
        if user_chapters is not None:  # None means see all
            if member.primary_chapter and member.primary_chapter not in user_chapters:
                ...
                try:
                    ...
                    if (hasattr(settings, 'national_board_chapter') and 
                        settings.national_board_chapter in user_chapters):
...
def get_member_membership_info(member_name):
    ...
    try:
        ...
        earliest_membership = frappe.get_value(
            "Membership",
            {"member": member_name},
            ["from_date", "membership_type"],
            order_by="from_date"
        )
        
        if earliest_membership:
            return {
                "member_since": earliest_membership[0],
                "membership_type": earliest_membership[1]
            }
        ...
        if current_membership:
            return {
                "member_since": current_membership[0],
                "membership_type": current_membership[1]
            }
...
def get_summary(data):
    ...
    return [
        {
            "value": total_new_members,
            "label": _("Total New Members"),
            "datatype": "Int",
            "color": "green"
        },
        {
            "value": very_new_count,
            "label": _("Very New (≤7 days)"),
            "datatype": "Int",
            "color": "blue"
        },
        {
            "value": recent_changes,
            "label": _("Recent Chapter Changes"),
            "datatype": "Int",
            "color": "orange"
        },
        {
        ...
        {
            "value": round(avg_days_active, 1),
            "label": _("Average Days Active"),
            "datatype": "Float"
        }
    ]
...
def get_chart_data(data):
    ...
    return {
        "data": {
            ...
            "datasets": [{
                "name": _("New Members"),
                "values": [week[1] for week in sorted_weeks]
            }]
        },
        "type": "line",
        "colors": ["#28a745"]
