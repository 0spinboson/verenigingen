...
def get_data(filters):
    ...
    if filters:
        ...
        if filters.get("to_date"):
            if "posting_date" in invoice_filters:
                invoice_filters["posting_date"] = ["between", [filters.get("from_date"), filters.get("to_date")]]
            else:
    ...
    for invoice in membership_invoices:
        ...
        if not customer_data[customer]["oldest_invoice_date"] or invoice.posting_date < customer_data[customer]["oldest_invoice_date"]:
            customer_data[customer]["oldest_invoice_date"] = invoice.posting_date
        
        if not customer_data[customer]["min_due_date"] or invoice.due_date < customer_data[customer]["min_due_date"]:
    ...
    try:
        ...
        for plan_detail in subscription_plans:
            ...
            if plan.item:
                ...
                if (item.item_group == "Membership" or 
                    "membership" in item.name.lower() or 
                    "lidmaatschap" in item.name.lower()):
                    return True
