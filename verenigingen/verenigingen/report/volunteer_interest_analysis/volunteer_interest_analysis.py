...
def get_volunteer_summary(data):
    ...
    most_common_avail = max(availability_counts.items(), key=lambda x: x[1])[0] if availability_counts else "N/A"
    
    return [
        {
            "value": total_interested,
            "label": _("Total Interested"),
            "datatype": "Int"
        },
        {
            "value": with_volunteer_record,
            "label": _("Volunteer Records Created"),
            "datatype": "Int"
        },
        {
        ...
        {
            "value": f"{(with_volunteer_record/total_interested*100):.1f}%" if total_interested > 0 else "0%",
            "label": _("Activation Rate"),
            "datatype": "Data"
        },
        {
            "value": total_hours,
            "label": _("Total Volunteer Hours"),
            "datatype": "Float"
        },
        {
            "value": most_common_avail,
            "label": _("Most Common Availability"),
            "datatype": "Data"
        }
    ]
...
def get_interest_distribution_chart(data):
    ...
    return {
        "data": {
            ...
            "datasets": [{
                "name": _("Volunteer Interest Areas"),
