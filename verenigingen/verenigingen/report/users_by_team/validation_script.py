...
def validate_report_dependencies():
    ...
    if "Team" not in missing_doctypes:
        ...
        if missing_team_fields:
            print(f"WARNING: The following fields are missing from Team doctype: {', '.join(missing_team_fields)}")
        else:
    ...
    if "Team Member Role" not in missing_doctypes:
        ...
        if missing_tmr_fields:
            print(f"WARNING: The following fields are missing from Team Member Role doctype: {', '.join(missing_tmr_fields)}")
        else:
    ...
    if "Team Role" not in missing_doctypes:
        ...
        if missing_tr_fields:
            print(f"WARNING: The following fields are missing from Team Role doctype: {', '.join(missing_tr_fields)}")
        else:
