...
class EBoekhoudenRESTIterator:
    ...
    def fetch_mutations_by_type(self, mutation_type: int, limit: int = 500, progress_callback=None) -> List[Dict[str, Any]]:
        """
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if isinstance(response_data, dict) and 'items' in response_data:
                        ...
                        frappe.log_error(f"Unexpected response format for type {mutation_type}", "REST Iterator")
                        break
                        
                else:
                    frappe.log_error(f"Failed to fetch mutations of type {mutation_type}: {response.status_code}", "REST Iterator")
                    break
                    
            except Exception as e:
                frappe.log_error(f"Error fetching mutations of type {mutation_type}: {str(e)}", "REST Iterator")
                break
    ...
    try:
        ...
        for mutation_type in test_types:
            try:
                ...
                mutations = iterator.fetch_mutations_by_type(mutation_type, limit=10, progress_callback=progress_callback)
...
def test_mutation_zero():
    ...
    try:
        ...
        result = {
            "mutation_0_exists": bool(mutation_0),
            "mutation_0_data": None
        }
...
def continue_rest_import_from_680():
    """Continue the REST import from mutation 680 where it left off"""
    try:
        # Simple approach: Create a new migration and import the remaining range
        from verenigingen.verenigingen.doctype.e_boekhouden_migration.e_boekhouden_migration import start_transaction_import
