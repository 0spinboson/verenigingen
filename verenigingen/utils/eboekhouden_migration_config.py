...
PAYMENT_ACCOUNT_CONFIG = {
    "bank_accounts": {
        ...
        "10470": {
            "name": "PayPal - info@veganisme.org",
            "type": "Bank",
            "mode_of_payment": "PayPal"
        },
        "10620": {
            "name": "ASN - 97.88.80.455",
            "type": "Bank", 
            "mode_of_payment": "Bank Transfer"
        }
    },
...
def is_payment_account(account_code):
    ...
    return (account_code in PAYMENT_ACCOUNT_CONFIG["bank_accounts"] or 
            account_code in PAYMENT_ACCOUNT_CONFIG["cash_accounts"])
...
def setup_payment_modes():
    ...
    return {
        "success": True,
        "created": created,
        "message": f"Created {len(created)} modes of payment"
    }
        ...
        for trans in transactions:
            ...
            if is_payment_account(account_code):
                ...
                payment_transactions.append({
                    ...
                    "will_create": "Payment Entry" if trans.get("relationId") else "Bank/Cash Transfer (Journal Entry)"
                })
            else:
