...
def create_initial_iban_history(member_name):
    ...
    try:
        ...
        if not hasattr(member, 'iban') or not member.iban:
            return {
                "success": False,
                "message": "Member has no IBAN to track"
            }
        
        # Check if IBAN history already exists
        existing_count = frappe.db.count('Member IBAN History', 
            filters={'parent': member_name})
        if existing_count > 0:
            return {
                "success": False,
                "message": "IBAN history already exists for this member"
            }
...
def get_iban_history(member_name):
    ...
    """
    try:
        history = frappe.get_all('Member IBAN History',
            filters={'parent': member_name},
            fields=['*'],
...
def track_iban_change(member_doc):
    ...
    try:
        ...
        if old_iban and old_iban != member_doc.iban:
            ...
            history_records = frappe.get_all('Member IBAN History',
                filters={'parent': member_doc.name, 'is_active': 1},
                fields=['name'])
            
            for record in history_records:
                frappe.db.set_value('Member IBAN History', record.name, {
                    'is_active': 0,
                    'to_date': today()
                })
            ...
            # Log the change
            frappe.logger().info(f"IBAN changed for member {member_doc.name} from {old_iban} to {member_doc.iban}")
