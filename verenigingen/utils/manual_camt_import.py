...
def process_manual_camt_document(xml_content, bank_account, company):
    ...
    try:
        ...
        try:
            ...
            return {
                "success": False,
                "message": f"Failed to parse CAMT document: {str(e)}"
            }
        ...
        return {
            "success": False,
            "message": f"Failed to process CAMT document: {str(e)}"
        }
...
def create_bank_transaction_from_sepa(sepa_transaction, bank_account, company):
    ...
    try:
        ...
        transaction_id = (
            getattr(sepa_transaction, 'bank_reference', None) or
            getattr(sepa_transaction, 'eref', None) or
            generate_transaction_hash(sepa_transaction)
        )
        ...
        bt.reference_number = getattr(sepa_transaction, 'eref', '')
...
def validate_camt_file(file_content):
    ...
    try:
        ...
        return {
            "success": False,
            "message": f"Invalid CAMT file: {str(e)}"
        }