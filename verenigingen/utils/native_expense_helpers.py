...
def get_volunteer_expense_approver(volunteer_name):
    ...
    try:
        ...
        frappe.log_error(f"Error getting expense approver for {volunteer_name}: {str(e)}", 
                        "Expense Approver Lookup Error")
    ...
    try:
        ...
        if approver:
            ...
            if old_approver != approver:
                ...
                frappe.logger().info(f"Updated expense approver for {volunteer.volunteer_name}: {old_approver} → {approver}")
        ...
        frappe.log_error(f"Error updating employee approver: {str(e)}", 
...
def fix_expense_approver_issues():
    ...
    return {
        "success": True,
        "fixed": fixed_count,
        "message": f"Fixed {fixed_count} expense approver issues"
    }
