...
def migration_fix_summary():
    ...
    # Root cause
    summary["root_cause"] = """The E-Boekhouden REST API migration was failing with error:
...
3. The error message was misleading - it appeared to be asking for an expense account on a Sales Invoice"""
    ...
    # Solution
    summary["solution"] = """Fixed by updating the Company's default_expense_account to an existing account:
...
- Changed to: '44470 - Onvoorziene kosten - NVV' (existing expense account)"""
    ...
    summary["verification"] = {
        ...
        "fiscal_years_2019_2024": frappe.db.count("Fiscal Year", {"year": ["in", ["2019", "2020", "2021", "2022", "2023", "2024"]]}),
...
def verify_migration_ready():
    ...
    fy_count = frappe.db.count("Fiscal Year", {"year": ["in", ["2019", "2020", "2021", "2022", "2023", "2024"]]})
    checks.append({
    ...
    checks.append({
        ...
        "status": f"✓ Pass: {eb_items} items created" if eb_items > 0 else "⚠ Warning: No EB- items found",
        "required": False
    })
    ...
    return {
        ...
        "checks": checks,
        "summary": "✓ System is ready for E-Boekhouden migration" if all_required_pass else "✗ Some required checks failed"
    }