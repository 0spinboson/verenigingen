...
class ExpensePermissionManager:
    ...
    def _can_approve_team_expense(self, expense_doc):
        ...
        team_leads = frappe.get_all("Team Member",
            filters={
                "parent": expense_doc.team, 
                "status": "Active", 
...
def get_expense_permission_query_conditions(user=None):
    ...
    if board_chapters:
        ...
        conditions.append(f"(`tabVolunteer Expense`.organization_type = 'Chapter' AND `tabVolunteer Expense`.chapter IN ('{chapter_conditions}'))")
        ...
        if team_conditions:
            conditions.append(f"(`tabVolunteer Expense`.organization_type = 'Team' AND ({' OR '.join(team_conditions)}))")
    ...
    if team_memberships:
        ...
        conditions.append(f"(`tabVolunteer Expense`.organization_type = 'Team' AND `tabVolunteer Expense`.team IN ('{team_conditions}'))")
