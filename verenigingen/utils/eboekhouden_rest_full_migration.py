        ...
        if company:
            ...
            if company_doc.enable_perpetual_inventory:
                ...
                if not company_doc.stock_received_but_not_billed:
                    return {
                        "success": False,
                        "error": _("Perpetual inventory is enabled but 'Stock Received But Not Billed' account is not set. Please either set this account in Company settings or disable perpetual inventory.")
                    }
            ...
            if amount != 0:  # Process both positive and negative amounts
                ...
                if mutation_type == 1:  # Purchase Invoice (FactuurOntvangen)
                    ...
                    if mutation_type == 7 and invoice_number and relation_id:
                        debug_info.append(f"Type 7 mutation with invoice {invoice_number} and relation {relation_id} - checking account type")
...
def check_accounts_and_items():
    ...
    try:
        ...
        import_item = frappe.db.get_value("Item", "E-Boekhouden Import Item", ["name", "item_name"], as_dict=True)
...
def debug_ledger_mappings():
    ...
    try:
        ...
        for ledger_id in ledger_ids:
            ...
            account_by_ebh = frappe.db.get_value("Account", {
                "company": "Ned Ver Vegan",
                "eboekhouden_grootboek_nummer": ledger_id
            }, ["name", "account_name", "account_type"], as_dict=True)
            
            # Check by account_number
            account_by_num = frappe.db.get_value("Account", {
                "company": "Ned Ver Vegan",
                "account_number": ledger_id
            }, ["name", "account_name", "account_type"], as_dict=True)
            
            # Check E-Boekhouden Item Mapping
            item_mapping = frappe.db.get_value("E-Boekhouden Item Mapping", {
                "account_code": ledger_id
            }, ["name", "item_code", "account_name"], as_dict=True)
    ...
    try:
        ...
        try:
            mappings = frappe.get_all("E-Boekhouden Account Mapping", 
                fields=["name", "account_code", "erpnext_account"],
                limit=50)
        ...
        for mutation_id in failing_ids:
            try:
                ...
                if mutation:
                    ...
                    debug_log.append(f"Fetched mutation {mutation_id}: {mutation.get('description', 'No description')[:50]}")
                else:
