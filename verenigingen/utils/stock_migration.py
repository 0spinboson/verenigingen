...
class StockTransactionMigrator:
    ...
    def migrate_stock_transactions(self, settings, date_from=None, date_to=None):
        ...
        try:
            ...
            # Debug logging
            print(f"Starting stock transaction migration...")
            ...
            # Set date range
            if not date_from:
                date_from = getdate(self.migration_doc.date_from) if hasattr(self.migration_doc, 'date_from') and self.migration_doc.date_from else getdate("2023-01-01")
            else:
                date_from = getdate(date_from)
                
            if not date_to:
                date_to = getdate(self.migration_doc.date_to) if hasattr(self.migration_doc, 'date_to') and self.migration_doc.date_to else getdate(today())
            else:
            ...
            print(f"Found {len(stock_accounts)} stock accounts: {', '.join([acc['account_number'] for acc in stock_accounts])}")
            ...
            while current_date <= date_to:
                ...
                if result["success"]:
                    ...
                    if stock_transactions:
                        print(f"Found {len(stock_transactions)} stock transactions in {current_date.strftime('%Y-%m')}")
            ...
            if account_code in stock_account_numbers:
                ...
                stock_transactions.append({
                    **transaction,
                    ...
                    'account_name': next((acc['account_name'] for acc in stock_accounts if acc['account_number'] == account_code), 'Unknown')
                })
            ...
            if invoices_data.get("items"):
                for invoice in invoices_data["items"][:3]:
                    ...
                    for key, value in invoice.items():
                        if any(keyword in key.lower() for keyword in ['product', 'item', 'inventory', 'stock', 'quantity', 'unit']):
            ...
            if mutations_data.get("items"):
                for mutation in mutations_data["items"][:5]:
                    ...
                    for key, value in mutation.items():
                        if any(keyword in key.lower() for keyword in ['quantity', 'product', 'item', 'inventory', 'stock', 'unit']):
...
def migrate_stock_transactions_standalone(migration_name, date_from=None, date_to=None, dry_run=True):
    ...
    try:
        ...
        if not settings.api_token:
            return {
                "success": False,
                "error": "E-Boekhouden Settings not configured"
            }
