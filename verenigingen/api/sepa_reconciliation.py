...
def identify_sepa_transactions():
    ...
    try:
        ...
        unprocessed_transactions = frappe.get_all("Bank Transaction",
            filters={
    ...
    try:
        ...
        if return_item.get("member_reference"):
            member = frappe.db.get_value("Member", 
                {"member_id": return_item["member_reference"]}, 
                ["name", "customer"]
            )
        ...
        if invoice:
            ...
            result = reverse_failed_sepa_payment(invoice[0], return_item)
...
def correlate_return_transactions():
    ...
    try:
        ...
        recent_returns = frappe.get_all("Bank Transaction",
            filters={
...
def get_sepa_reconciliation_dashboard():
    ...
    try:
        ...
        linked_transactions = frappe.get_all("Bank Transaction",
            filters={
                "custom_sepa_batch": ["is", "set"],
                "date": [">=", add_days(getdate(), -30)]
            },
