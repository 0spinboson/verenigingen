...
from frappe.utils import today, add_months, getdate, flt, format_date
...
def get_dashboard_data(member=None):
    ...
    total_paid_year = frappe.db.sql("""
        ...
        AND docstatus = 1
    """, (member_doc.customer, current_year))[0][0] if member_doc.customer else 0
...
def get_payment_history(member=None, year=None, status=None):
    ...
    payments = frappe.get_all(
        'Payment Entry',
        filters=filters,
        fields=['name', 'posting_date as date', 'paid_amount as amount', 
                'reference_no', 'remarks', 'mode_of_payment'],
    ...
    """.format(conditions=invoice_conditions), params, as_dict=True)
...
def get_mandate_history(member=None):
    ...
    mandates = frappe.get_all(
        ...
        fields=['name', 'mandate_id', 'iban', 'bic', 'status', 
                'sign_date', 'expiry_date', 'cancelled_date', 
                'cancellation_reason', 'creation'],
...
def retry_failed_payment(invoice_id):
    ...
    existing_retry = frappe.db.exists('SEPA Payment Retry', {
        'invoice': invoice_id,
