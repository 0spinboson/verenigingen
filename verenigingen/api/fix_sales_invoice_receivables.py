...
def fix_existing_sales_invoice_receivables(company=None):
    ...
    try:
        ...
        wrong_account = frappe.db.get_value("Account", 
            {"account_number": "13500", "company": company}, "name")  # Te ontvangen contributies
        correct_account = frappe.db.get_value("Account", 
            {"account_number": "13900", "company": company}, "name")  # Te ontvangen bedragen
        ...
        wrong_invoices = frappe.get_all("Sales Invoice",
            filters={
                "company": company,
...
def check_sales_invoice_receivables(company=None):
    ...
    try:
        ...
        contrib_account = frappe.db.get_value("Account", 
            {"account_number": "13500", "company": company}, ["name", "account_name"], as_dict=True)
        amounts_account = frappe.db.get_value("Account", 
            {"account_number": "13900", "company": company}, ["name", "account_name"], as_dict=True)
        
        # Count sales invoices by receivable account
        contrib_count = frappe.db.count("Sales Invoice", {
            "company": company,
            "debit_to": contrib_account.name if contrib_account else "",
            "docstatus": 1
        })
        
        amounts_count = frappe.db.count("Sales Invoice", {
            "company": company, 
            "debit_to": amounts_account.name if amounts_account else "",
            "docstatus": 1
        })
