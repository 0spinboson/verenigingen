...
def create_optimal_batches(target_date=None, config=None):
    ...
    try:
        ...
        for group_index, batch_group in enumerate(batch_groups):
            batch_doc = create_dd_batch_document(
                batch_group, 
                target_date, 
                group_index + 1,
                batch_config
...
def create_optimal_batch_groups(analysis, config):
    ...
    if analysis["by_priority"].get("High"):
        priority_batches = create_priority_batches(
            analysis["by_priority"]["High"], 
            config
        )
        ...
        remaining_invoices = [inv for inv in remaining_invoices 
                            if inv["invoice"] not in processed_invoices]
    
    # Strategy 2: Create customer-consolidated batches
    customer_batches = create_customer_consolidated_batches(
        remaining_invoices, 
        config
    )
    ...
    amount_batches = create_amount_optimized_batches(
        remaining_invoices, 
        config
    )
...
def create_priority_batches(priority_invoices, config):
    ...
    for invoice in sorted(priority_invoices, key=lambda x: flt(x["amount"]), reverse=True):
        ...
        if (len(current_batch) >= config["max_invoices_per_batch"] or
            current_amount + invoice_amount > config["max_amount_per_batch"]):
...
def create_customer_consolidated_batches(invoices, config):
    ...
    single_invoice_customers = {k: v for k, v in customer_groups.items() if len(v) == 1}
    ...
    for customer, customer_invoices in multi_invoice_customers.items():
        ...
        if (len(customer_invoices) >= config["min_invoices_per_batch"] and
            customer_total <= config["max_amount_per_batch"]):
            batches.append(customer_invoices)
            processed_customers.add(customer)
        else:
            # Try to fit customer invoices in current batch
            if (len(current_batch) + len(customer_invoices) <= config["max_invoices_per_batch"] and
                current_amount + customer_total <= config["max_amount_per_batch"]):
...
def create_dd_batch_document(batch_invoices, target_date, batch_number, config):
    ...
    frappe.logger().info(f"Created batch {batch_doc.name} with {len(batch_invoices)} invoices (€{total_amount})")
    return batch_doc
...
def generate_optimization_report(original_invoices, created_batches, config):
    ...
    # Risk assessment
    high_risk_batches = sum(1 for batch in created_batches 
                           if flt(batch.total_amount) > 4000 or batch.entry_count > 25)
    
    risk_distribution = {
        "high_risk_batches": high_risk_batches,
        "medium_risk_batches": len([b for b in created_batches 
                                   if 2000 < flt(b.total_amount) <= 4000]),
        "low_risk_batches": len([b for b in created_batches 
                                if flt(b.total_amount) <= 2000])
    }
        ...
        "batch_details": [
            {
                ...
                "risk_level": "High" if flt(batch.total_amount) > 4000 or batch.entry_count > 25
                            else "Medium" if flt(batch.total_amount) > 2000 
                            else "Low"
            }
...
def get_batching_preview(config=None):
    ...
    for i, group in enumerate(batch_groups):
        ...
        preview.append({
            ...
            "risk_level": "High" if group_total > 4000 or len(group) > 25
                         else "Medium" if group_total > 2000 
                         else "Low",
...
def update_batch_optimization_config(new_config):
    ...
    return {
        "success": True,
        "message": "Batch optimization configuration updated",
        "config": new_config
    }