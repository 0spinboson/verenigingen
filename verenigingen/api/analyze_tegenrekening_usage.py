...
def analyze_tegenrekening_patterns():
    ...
    try:
        ...
        tegenrekening_stats = defaultdict(lambda: {
            'count': 0,
            'total_amount': 0,
            'descriptions': set(),
        ...
        sorted_accounts = sorted(tegenrekening_stats.items(), 
        ...
        for account_code, stats in sorted_accounts[:20]:  # Top 20
            ...
            response.append(f"  Sample descriptions:")
            for desc in stats['sample_descriptions'][:3]:
                response.append(f"    - {desc}")
        
        response.append(f"\n=== ACCOUNT PATTERNS ===")
        sorted_patterns = sorted(account_patterns.items(), 
        ...
        for account_code, stats in sorted_accounts[:10]:
            ...
            mapping_suggestions.append({
                ...
                'sample_description': stats['sample_descriptions'][0] if stats['sample_descriptions'] else ""
            })
        
        response.append("Top mapping suggestions:")
        for suggestion in mapping_suggestions:
            response.append(f"  {suggestion['account_code']} → '{suggestion['suggested_item_name']}' ({suggestion['usage_count']} uses)")
