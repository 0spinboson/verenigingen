...
def create_payment_entry_with_duplicate_check(invoice_name: str, amount: float, payment_data: Dict) -> Dict:
    ...
    if not invoice_total:
        raise frappe.ValidationError(
            _("Invoice {0} not found or has no total amount").format(invoice_name)
        )
    
    if total_allocated >= flt(invoice_total):
        raise frappe.ValidationError(
            _("Invoice {0} already fully paid. Total allocated: {1}, Invoice total: {2}")
            .format(invoice_name, total_allocated, invoice_total)
        )
    
    if total_allocated + amount > flt(invoice_total):
        raise frappe.ValidationError(
            _("Payment amount {0} would exceed invoice total. Already allocated: {1}, Invoice total: {2}")
...
def check_batch_processing_status(batch_name: str, transaction_name: str) -> None:
    ...
    existing_payments = frappe.get_all(
        "Payment Entry",
        filters={
            "custom_sepa_batch": batch_name,
            "docstatus": 1  # Submitted payments only
        },
    ...
    if existing_payments:
        ...
        if other_transactions:
            raise frappe.ValidationError(
                _("SEPA batch {0} has already been processed with different bank transaction(s): {1}")
                .format(batch_name, ", ".join([p.custom_bank_transaction for p in other_transactions[:3]]))
            )
        
        # Check if same transaction is being reprocessed
        same_transaction_payments = [p for p in existing_payments if p.custom_bank_transaction == transaction_name]
        if same_transaction_payments:
            raise frappe.ValidationError(
                _("Bank transaction {0} has already been used to process SEPA batch {1}")
                .format(transaction_name, batch_name)
            )
...
def detect_incomplete_reversals() -> List[Dict]:
    ...
    return_records = frappe.get_all(
        "SEPA Return Record",
        fields=["name", "original_payment", "member", "amount", "return_reason"]
    )
