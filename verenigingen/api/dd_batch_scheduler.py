...
from frappe.utils import getdate, add_days, get_weekday, now_datetime
from verenigingen.api.dd_batch_optimizer import create_optimal_batches, DEFAULT_CONFIG
import json
...
def send_batch_creation_notification(result):
    ...
    try:
        ...
        finance_managers = frappe.get_all(
            ...
            filters={
                "enabled": 1,
                "name": ["in", frappe.get_list("Has Role", 
...
def create_system_notification(result):
    ...
    try:
        ...
        finance_users = frappe.get_list("Has Role", 
            filters={"role": "Finance Manager"}, 
            pluck="parent"
        )
...
def toggle_auto_batch_creation(enabled):
    ...
    return {
        "success": True,
        "message": f"Auto batch creation {action}",
