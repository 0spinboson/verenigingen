...
def quick_approve_member(member_name, chapter_name=None):
    ...
    if not chapter_name:
        ...
        if not member_chapter:
            ...
            chapter_member = frappe.db.get_value("Chapter Member", 
                {"member": member_name, "status": "Pending"}, "parent")
            if chapter_member:
    ...
    try:
        ...
        if result.get('success'):
            ...
            return {
                "success": True,
                "message": _("Member approved successfully"),
                "member_name": member_name
            }
        else:
            return {
...
def debug_mt940_import(import_name):
    ...
    try:
        ...
        basic_info = {
            ...
            "import_date": str(import_doc.import_date) if hasattr(import_doc, 'import_date') and import_doc.import_date else None,
            "statement_from_date": str(import_doc.statement_from_date) if hasattr(import_doc, 'statement_from_date') and import_doc.statement_from_date else None,
            "statement_to_date": str(import_doc.statement_to_date) if hasattr(import_doc, 'statement_to_date') and import_doc.statement_to_date else None,
            "descriptive_name": import_doc.descriptive_name if hasattr(import_doc, 'descriptive_name') else None,
            "transactions_created": import_doc.transactions_created if hasattr(import_doc, 'transactions_created') else 0,
            "transactions_skipped": import_doc.transactions_skipped if hasattr(import_doc, 'transactions_skipped') else 0,
        ...
        return {
            ...
            "file_info": file_info,
            "mt940_library": {
                "available": mt940_available,
                "version": mt940_version
            },
                ...
                for j, transaction in enumerate(statement_transactions[:3]):
                    try:
                        ...
                        trans_info.update({
                            ...
                            "sepa_data_sample": {k: v[:50] if isinstance(v, str) else v for k, v in sepa_data.items() if k != 'raw_sepa'}
                        })
    ...
    for chapter_info in user_chapters:
        ...
        if overdue_apps.count > 0:
            notifications.append({
                ...
                "message": _("{0} membership applications are overdue for review").format(overdue_apps.count),
...
def get_chapter_quick_stats(chapter_name):
    ...
    # Board member count
    board_count = frappe.db.count("Chapter Board Member", {
        "parent": chapter_name,
        "is_active": 1
    })
    
    # Recent activity count (last 7 days)
    recent_activity = frappe.db.count("Comment", {
...
def reject_member_application(member_name, chapter_name, reason=None):
    ...
    try:
        ...
        chapter_member = frappe.db.get_value("Chapter Member", 
            {"member": member_name, "parent": chapter_name, "status": "Pending"}, 
            "name")
...
def debug_dashboard_access():
    ...
    try:
        ...
        from verenigingen.templates.pages.chapter_dashboard import get_user_board_chapters, get_context
...
def get_active_members_count(chapter=None):
    ...
    if not chapter:
        ...
        for ch in user_chapters:
            count = frappe.db.count("Chapter Member", {
                "parent": ch['chapter_name'],
                "enabled": 1,
                "status": ["in", ["Active", ""]]
            })
            total += count
...
def get_board_members_count(chapter=None):
    ...
    if not chapter:
        ...
        total = 0
        for ch in user_chapters:
            count = frappe.db.count("Chapter Board Member", {
                "parent": ch['chapter_name'],
                "is_active": 1
            })
            total += count
...
def create_chapter_dashboard_doc():
    ...
    dashboard = frappe.get_doc({
        ...
        "cards": [
            {
                "card": "Active Chapter Members",
                "width": "Half"
            },
            {
                "card": "Pending Member Applications", 
                "width": "Half"
            },
            {
                "card": "Active Board Members",
                "width": "Half" 
            },
            {
                "card": "New Members This Month",
                "width": "Half"
            }
        ],
        "charts": [
            {
                "chart": "Chapter Member Status",
                "width": "Half"
            },
            {
                "chart": "Chapter Member Count", 
                "width": "Half"
            }
        ]
    })
...
def create_simple_dashboard():
    ...
    try:
        ...
        existing_chart = frappe.get_all("Dashboard Chart", 
            filters={"is_public": 1}, 
            fields=["name"], 
            limit=1)
...
def fix_chart_currency_display():
    ...
    try:
        ...
        return {
            "success": True,
            "message": "Fixed currency display in charts",
...
def add_existing_cards_to_dashboard():
    ...
    try:
        ...
        existing_cards = frappe.get_all("Number Card", 
            filters={"is_public": 1}, 
            fields=["name", "label"], 
            limit=4)
...
def use_existing_working_charts():
    ...
    try:
        ...
        working_charts = frappe.get_all("Dashboard Chart", 
            filters={
                "is_public": 1,
                "chart_type": "Report"  # Report charts tend to work better
            }, 
...
def create_proper_chapter_charts():
    ...
    try:
        ...
        dashboard.append("charts", {
            "chart": "Member Status Distribution",
            "width": "Half"
        })
        dashboard.append("charts", {
            "chart": "Chapter Distribution", 
            "width": "Half"
        })
...
def debug_number_cards():
    ...
    try:
        ...
        return {
            "success": True,
            "results": results,
            "user_has_access": len(results["user_chapters"]) > 0
        }
...
def create_chapter_member_charts():
    ...
    try:
        ...
        dashboard.append("charts", {
            "chart": "Member Status Overview",
            "width": "Half"
        })
        dashboard.append("charts", {
            "chart": "Chapter Activity",
            "width": "Half"
        })
...
def test_dashboard_access():
    ...
    try:
        ...
        return {
            ...
            "number_cards": {
                "working": all(isinstance(result, dict) and "value" in result for result in card_results.values()),
...
def clean_dashboard_completely():
    ...
    try:
        ...
        our_cards = frappe.get_all("Number Card", 
...
def get_filed_expense_claims_count(chapter=None):
    ...
    if not chapter:
        ...
        total = frappe.db.count("Expense Claim", {
            "approval_status": ["!=", "Draft"]
        })
        return {"value": total, "fieldtype": "Data"}
    else:
        # For specific chapter (if we add chapter filtering later)
        count = frappe.db.count("Expense Claim", {
            "approval_status": ["!=", "Draft"]
        })
        return {"value": count, "fieldtype": "Data"}
...
def get_volunteer_expenses_count(chapter=None):
    ...
    if not chapter:
        ...
        total = frappe.db.count("Volunteer Expense", {
            "status": "Submitted"
        })
        return {"value": total, "fieldtype": "Data"}
    else:
        count = frappe.db.count("Volunteer Expense", {
            "status": "Submitted"
        })
        return {"value": count, "fieldtype": "Data"}
