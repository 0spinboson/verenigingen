...
def create_missing_sepa_mandates(dry_run=True):
    ...
    results = {
        "found": len(members_needing_mandates),
        "created": 0,
        "errors": [],
        "mandates": []
    }
    ...
    for member in members_needing_mandates:
        try:
            if dry_run:
                ...
                existing_today = frappe.db.count("SEPA Mandate", {
...
def fix_specific_member_sepa_mandate(member_name):
    ...
    existing_active = frappe.db.exists("SEPA Mandate", {
        "member": member_name,
        "status": "Active",
        "is_active": 1
    })
    ...
    if existing_active:
        return {
            "success": False,
            "message": _("Member already has an active SEPA mandate")
        }
    ...
    for error_info in result.get("results", {}).get("errors", []):
        if error_info.get("member") == member_name:
            return {
                "success": False,
                "message": f"Error creating mandate: {error_info.get('error')}"
            }
    
    return {
        "success": False,
        "message": "Member was not processed - please check the criteria"
    }