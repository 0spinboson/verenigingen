...
def get_invoice_mandate_info(invoice):
    ...
    return {
        'valid': False,
        'error': _('No active SEPA mandate found')
    }
...
def validate_invoice_mandate(invoice, member):
    ...
    try:
        ...
        if not mandate:
            return {
                'valid': False,
                'error': _('No active SEPA mandate')
            }
    ...
    for inv in batch.invoices:
        ...
        if not inv.iban:
            ...
            analytics['issues'].append({
                'invoice': inv.invoice,
                'member': inv.member_name,
                'issue': 'Missing mandate reference'
            })
...
def preview_sepa_xml(batch_name):
    ...
    preview = {
        ...
        'payment_info': {
            ...
            'creditor_name': frappe.db.get_single_value('Vereiningen Settings', 'company_name'),
            'creditor_iban': frappe.db.get_single_value('Vereiningen Settings', 'company_iban'),
            'creditor_id': frappe.db.get_single_value('Vereiningen Settings', 'creditor_id')
        },
        'transactions': []
    }
