...
def generate_test_membership_types():
        ...
        try:
            ...
            if not membership_type.subscription_plan:
                try:
                    ...
                    frappe.log_error(f"Failed to create subscription plan for {membership_type.name}: {str(e)}")
    ...
    return {
        "success": True,
        "summary": summary,
        "membership_types": created_types,
        "errors": errors
    }
...
def cleanup_test_membership_types():
    ...
    test_types = frappe.get_all("Membership Type", 
        ...
        try:
            ...
            if mt.subscription_plan:
                if frappe.db.exists("Subscription Plan", mt.subscription_plan):
                    ...
                    plan_items = frappe.get_all("Subscription Plan Detail",
                        filters={"parent": mt.subscription_plan},
                        fields=["item"]
                    )
