...
def get_mapping_summary():
    ...
    journal_mappings = frappe.db.count("E-Boekhouden Account Mapping", {
        "is_active": 1,
        "document_type": "Journal Entry"
    })
    
    purchase_mappings = frappe.db.count("E-Boekhouden Account Mapping", {
        "is_active": 1,
        "document_type": "Purchase Invoice"
    })
    
    # Get sample mappings
    sample_mappings = frappe.get_all("E-Boekhouden Account Mapping",
        filters={"is_active": 1},
...
def test_mutation_mapping(mutation_nr=None, account_code=None, description=None):
    ...
    if mapping.get("name"):
        ...
        if mapping_doc.account_code == account_code:
            ...
            mapping_details["reasons"].append(f"Account in range {mapping_doc.account_range_start} - {mapping_doc.account_range_end}")
