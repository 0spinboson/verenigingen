...
def validate_sepa_mandates(batch):
    ...
    for invoice_item in batch.invoices:
        ...
        if not mandate:
            issues.append(f"SEPA mandate {invoice_item.mandate_reference} not found for invoice {invoice_item.invoice}")
        elif mandate.status != "Active" or not mandate.is_active:
            issues.append(f"SEPA mandate {invoice_item.mandate_reference} is not active for invoice {invoice_item.invoice}")
...
def approve_batch(batch_name, approval_notes=None):
    ...
    try:
        ...
        if not validation_result.get("valid"):
            frappe.throw(_("Batch validation failed: {0}").format(
...
def reject_batch(batch_name, rejection_reason):
    ...
    try:
        ...
        return {
            "success": True,
            "message": "Batch rejected with reason provided"
        }
...
def get_batch_approval_history(batch_name):
    ...
    try:
        ...
        frappe.log_error(f"Error getting approval history for batch {batch_name}: {str(e)}", "DD Batch History Error")
        return {
            "success": False,
            "error": str(e)
        }
...
def trigger_sepa_generation(batch_name):
    ...
    try:
        ...
        return {
            "success": True,
            "sepa_file": sepa_file,
            "message": "SEPA file generated successfully"
        }
        
    except Exception as e:
        frappe.log_error(f"Error generating SEPA file for batch {batch_name}: {str(e)}", "DD SEPA Generation Error")
    ...
    try:
        ...
        batches = frappe.get_all(
            ...
            fields=[
                ...
                "entry_count", "approval_status", "risk_level", "creation"
            ],
