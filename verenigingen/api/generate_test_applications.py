...
def generate_test_members():
    ...
    existing_test_apps = frappe.get_all("Membership Application", 
...
def cleanup_test_applications():
    ...
    """
    test_applications = frappe.get_all("Membership Application",
        filters={"email": ["like", "%@email.nl"]},
        fields=["name"]
    )
    ...
    for app in test_applications:
        try:
            ...
            member_exists = frappe.db.exists("Member", {"email": frappe.db.get_value("Membership Application", app.name, "email")})
