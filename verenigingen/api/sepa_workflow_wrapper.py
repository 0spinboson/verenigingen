...
def execute_complete_reconciliation(workflow_data: dict) -> dict:
    ...
    if not bank_transaction or not sepa_batch:
        return {
            "success": False,
            "error": "Bank transaction and SEPA batch are required"
        }
...
def process_complete_return_file(return_file_content: str, file_name: str = "") -> dict:
    ...
    try:
        ...
        if not acquire_processing_lock("return_file", file_hash):
            return {
                "success": False,
                "error": "Return file is currently being processed by another user"
            }
...
def run_comprehensive_sepa_audit() -> dict:
    ...
    audit_results["audit_sections"].append({
        ...
        "recommendation": "Review and fix orphaned payment entries" if orphaned_payments else "No action needed"
    })
    ...
    audit_results["audit_sections"].append({
        ...
        "recommendation": "Complete failed payment reversals" if incomplete_reversals else "No action needed"
    })
    ...
    audit_results["audit_sections"].append({
        ...
        "recommendation": "Process identified SEPA transactions" if unprocessed_transactions.get("potential_matches") else "All transactions processed"
    })
    ...
    audit_results["audit_sections"].append({
        ...
        "recommendation": "Investigate unmatched return transactions" if unmatched_returns.get("unmatched_returns") else "All returns matched"
    })
...
def generate_duplicate_prevention_report() -> dict:
    ...
    batch_reprocess_attempts = frappe.get_all(
        "Error Log",
        filters={
            "error": ["like", "%already been processed%"],
            "creation": [">=", getdate()]
        },
