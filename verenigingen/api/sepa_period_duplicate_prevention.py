...
Prevents double invoicing for the same subscription period
"""
...
def check_period_invoicing_duplicates(member_name: str, period_start: str, period_end: str, membership_type: str = None) -> Dict:
    """
    ...
    try:
        ...
        existing_invoices = frappe.get_all(
            ...
            fields=[
                ...
                "custom_period_start", "custom_period_end", "custom_membership_type"
            ]
        )
        ...
        for invoice in existing_invoices:
            ...
            is_membership_invoice = any(
                _is_membership_item(item) for item in invoice_items
            )
        ...
        for invoice in membership_invoices:
            ...
            if _periods_overlap(
                ...
                invoice.get("custom_period_start"), invoice.get("custom_period_end")
            ):
        ...
        if has_duplicates and _is_strict_mode_enabled():
            raise frappe.ValidationError(
                _("Member {0} already has invoices for period {1} to {2}. Found {3} overlapping invoice(s): {4}")
                .format(
                    member.full_name,
                    period_start,
                    period_end,
...
def check_subscription_period_duplicates(subscription_name: str, period_start: str, period_end: str) -> Dict:
    """
    Check if subscription has already generated invoices for a specific period
    ...
    try:
        ...
        # Get all invoices generated by this subscription
        subscription_invoices = frappe.get_all(
            ...
            fields=[
                "name", "posting_date", "from_date", "to_date", 
        ...
        for invoice in subscription_invoices:
            invoice_start = invoice.get("from_date") or invoice.posting_date
            invoice_end = invoice.get("to_date") or invoice.posting_date
        ...
        return {
            ...
            "total_subscription_invoices": len(subscription_invoices)
        }
        
    except Exception as e:
        frappe.log_error(f"Error checking subscription period duplicates: {str(e)}")
        return {
            "has_duplicates": False,
            "error": str(e),
            "subscription": subscription_name
...
def prevent_sepa_batch_period_duplicates(batch_name: str) -> Dict:
    ...
    try:
        ...
        if has_conflicts and _is_strict_mode_enabled():
            raise frappe.ValidationError(
                _("SEPA batch {0} contains {1} invoice(s) with period conflicts. Cannot process batch with duplicate periods.")
...
def _is_membership_item(item: Dict) -> bool:
    ...
    membership_keywords = [
        "membership", "lidmaatschap", "contributie", "subscription",
        "fee", "dues", "annual", "monthly", "yearly"
    ]
...
def generate_membership_billing_periods(member_name: str, start_date: str, billing_frequency: str = "Monthly") -> List[Dict]:
    """
...
def generate_period_duplicate_report(date_range: str = "Last 3 Months") -> Dict:
    ...
    top_duplicates = sorted(
        [(customer, len(data["duplicates"])) for customer, data in customer_analysis.items() if data["duplicates"]],
        key=lambda x: x[1],
        reverse=True
    )[:10]
