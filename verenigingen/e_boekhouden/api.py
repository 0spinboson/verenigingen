...
def get_migration_config_status():
    ...
    try:
        ...
        mappings = frappe.get_all(
            ...
            fields=['name', 'account_code', 'account_name', 'target_account_type', 
                   'transaction_category', 'priority', 'description']
        )
...
def get_staged_data_summary():
    ...
    try:
        ...
        return {
            ...
            'date_range': {
                'from': staged_data.get('from_date'),
                'to': staged_data.get('to_date')
            }
        }
...
def export_migration_config():
    ...
    try:
        ...
        mappings = frappe.get_all(
            "E-Boekhouden Account Mapping",
            fields=['account_code', 'account_name', 'target_account_type', 
                   'target_document_type', 'transaction_category', 'priority', 
                   'description', 'account_code_start', 'account_code_end',
                   'description_pattern']
        )
...
def import_migration_config(config):
    ...
    try:
        ...
        for mapping_data in config.get('mappings', []):
            ...
            existing = frappe.db.exists("E-Boekhouden Account Mapping", {
                "account_code": mapping_data.get('account_code')
            })
            ...
            # Update fields
            for field in ['account_code', 'account_name', 'target_account_type',
                         'target_document_type', 'transaction_category', 'priority',
                         'description', 'account_code_start', 'account_code_end',
                         'description_pattern']:
    ...
    try:
        ...
        for update in updates:
            try:
                ...
                frappe.log_error(f"Error updating mapping {update.get('mapping_id')}: {str(e)}", "E-Boekhouden")
...
def suggest_account_mappings():
    ...
    try:
        ...
        accounts = staged_data.get('accounts', [])
        existing_mappings = {m['account_code']: m for m in frappe.get_all(
            "E-Boekhouden Account Mapping",
            fields=['account_code']
        )}
        ...
        for account in accounts:
            ...
            if suggested_type:
                ...
                confidence = 'high'
                if account['code'].startswith('1') and not (account['code'].startswith('10') or account['code'].startswith('11')):
...
def apply_suggested_mappings(suggestions):
    ...
    try:
        ...
        for suggestion in suggestions:
            # Check if mapping already exists
            if not frappe.db.exists("E-Boekhouden Account Mapping", {
