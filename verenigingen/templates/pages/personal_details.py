...
from frappe.utils import today, now, cint, validate_email_address
import re
import os
    ...
    for field, change in changes.items():
        if field == 'image':
            if change['action'] == 'remove':
                change_descriptions.append(f"Removed profile image")
            elif change['action'] == 'upload':
                change_descriptions.append(f"Updated profile image")
        else:
...
def prepare_success_message(changes):
    ...
    preference_changes = sum(1 for field in changes.keys() if field in ['pronouns', 'allow_directory_listing', 'allow_photo_usage'])
