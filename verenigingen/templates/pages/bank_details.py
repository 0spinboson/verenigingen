...
def update_bank_details():
    ...
    try:
        ...
        frappe.logger().info(f"=== BANK DETAILS UPDATE START ===")
        ...
        try:
            ...
            with open("/tmp/bank_details_debug.log", "a") as f:
                f.write(f"DEBUG: Parsed - IBAN: {new_iban}, BIC: {new_bic}, Holder: {new_account_holder}, DD: {enable_dd}\n")
            
            frappe.logger().info(f"Parsed form data - IBAN: {new_iban}, BIC: {new_bic}, Account holder: {new_account_holder}, Enable DD: {enable_dd}")
        ...
        with open("/tmp/bank_details_debug.log", "a") as f:
            f.write(f"DEBUG: Bank details changed: {bank_details_changed}, Current mandate: {current_mandate}\n")
        ...
        # Prepare context for confirmation page (serialize member object)
        context = {
            ...
            if result.get('mandate_result'):
                ...
                if mandate_result.get('success'):
                    ...
                    if action_needed == "create_mandate":
                        if method == "direct":
                            success_messages.append(_("A new SEPA Direct Debit mandate has been created and activated immediately"))
                            if mandate_id:
                                success_messages.append(_("Mandate ID: {0}").format(mandate_id))
                        else:
                            success_messages.append(_("A new SEPA Direct Debit mandate will be created within 24 hours"))
                    elif action_needed == "replace_mandate":
                        if method == "direct":
                            success_messages.append(_("Your SEPA mandate has been updated with the new bank details"))
                        else:
        ...
        frappe.logger().error(f"=== BANK DETAILS UPDATE FAILED ===")
        frappe.throw(_("An error occurred while processing your bank details. Please try again or contact support. Error: {0}").format(str(e)))
...
def test_bank_details_api():
    ...
    try:
        ...
        return {
            "success": False,
            "error": str(e),
            "message": "Bank details API test failed"
        }
...
def debug_form_submission():
    ...
    try:
        frappe.logger().info(f"=== DEBUG FORM SUBMISSION ===")
        frappe.logger().info(f"User: {frappe.session.user}")
