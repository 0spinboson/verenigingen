...
def get_context(context):
    ...
    frappe.logger().info(f"=== BANK DETAILS CONFIRM ===")
        ...
        if enable_dd:
            if action_needed == "create_mandate":
                if processing_method == "direct":
                    message = _("Bank details updated and SEPA Direct Debit enabled successfully! Your mandate is active immediately.")
                else:
                ...
                if processing_method == "direct":
                    message = _("Bank details and SEPA mandate updated successfully! Changes are active immediately.")
                else:
            ...
            if action_needed == "cancel_mandate":
                if processing_method == "direct":
                    ...
                    message = _("Bank details updated successfully. SEPA Direct Debit will be disabled within 24 hours.")
            else:
...
def handle_sepa_mandate_changes(member_name, action_needed, current_mandate, new_iban, new_bic, new_account_holder):
    ...
    try:
        if action_needed == "create_mandate":
            ...
            if current_mandate:
                update_result = update_existing_mandate(current_mandate['name'], new_iban, new_bic, new_account_holder)
...
def create_mandate_pending_record(member_name, iban, bic, account_holder):
    ...
    try:
        ...
        mandate = frappe.get_doc({
            ...
            "scheme": "SEPA",
            "sign_date": today(),
            "first_collection_date": frappe.utils.add_days(today(), 5),  # SEPA requires 5 business days notice
...
def cancel_existing_mandate(mandate_name):
    ...
    try:
        ...
        # If permission denied, schedule for async processing
        frappe.log_error(
            f"SEPA_MANDATE_CANCEL|{mandate_name}|{now()}",
            "SEPA Mandate Cancellation Pending"
        )
        return {"success": True, "method": "async"}
...
def test_sepa_integration():
    ...
    try:
        ...
        from verenigingen.templates.pages.bank_details import validate_iban_format, derive_bic_from_dutch_iban
        ...
        results["tests"].append({
            "test": "BIC derivation function",
            "passed": bic == "ABNANL2A",
            "details": f"Derived BIC: {bic}"
        })
...
def get_current_user_info():
    ...
    try:
        ...
        # Get user details
        user_details = frappe.db.get_value("User", current_user, ["name", "email", "full_name", "enabled"], as_dict=True)
...
def process_bank_details_update_direct(member_name, new_iban, new_bic, new_account_holder, enable_dd, action_needed, current_mandate):
