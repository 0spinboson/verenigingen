...
def get_configuration_status():
    ...
    try:
        ...
        status["ready_for_migration"] = (
            status["api_configured"] and 
            status["default_company"] and
            (status["has_staged_data"] or mapping_count > 0)
        )
...
def stage_data_for_configuration():
    ...
    try:
        ...
        from verenigingen.verenigingen.doctype.e_boekhouden_migration.e_boekhouden_migration import stage_eboekhouden_data
        ...
        if result.get("success"):
            ...
            return {
                "success": False,
                "error": result.get("error", "Failed to stage data")
            }
...
def get_staging_data():
    ...
    try:
        ...
        if not migration:
            return {
                "success": False,
                "error": "No staged data found. Please stage data first."
            }
        
        # Get the staging data from the migration
        from verenigingen.verenigingen.doctype.e_boekhouden_migration.e_boekhouden_migration import get_staging_data_for_review
...
def get_account_mappings():
    ...
    try:
        mappings = frappe.get_all(
            ...
            fields=["name", "account_code", "account_name", "account_type", 
                   "confidence_level", "notes", "creation", "modified"],
...
def preview_migration_impact():
    ...
    try:
        ...
        from verenigingen.verenigingen.doctype.e_boekhouden_migration.e_boekhouden_migration import preview_mapping_impact
...
def start_configured_migration():
    ...
    try:
        ...
        # Start the migration
        from verenigingen.verenigingen.doctype.e_boekhouden_migration.e_boekhouden_migration import approve_and_continue_migration
        ...
        if result.get("success"):
            ...
            return {
                "success": False,
                "error": result.get("error", "Failed to start migration")
            }
...
def import_configuration(config_json):
    ...
    try:
        ...
        return {
            ...
            "errors": errors,
            "message": f"Imported {imported_count} mappings" + (f" with {len(errors)} errors" if errors else "")
        }
