...
class StandaloneMemberIdentityValidator:
    ...
    def test_name_similarity_cases(self):
        ...
        for name1, name2, expected_range in test_cases:
            ...
            print(f"{status} '{name1}' vs '{name2}': {similarity:.3f} (expected {expected_min:.1f}-{expected_max:.1f})")
    ...
    def test_risk_score_calculation(self):
        ...
        test_cases = [
            ...
            ({
                ...
                'iban_match': True,
                'birth_date_match': False
            }, (0.6, 0.7)),  # High risk due to IBAN match
        ]
...
def test_edge_case_scenarios():
    ...
    email_similarity = difflib.SequenceMatcher(
        None, 
        john_amsterdam['email'], 
        john_rotterdam['email']
    ).ratio()
    
    iban_match = (validator._normalize_iban(john_amsterdam['iban']) == 
                  validator._normalize_iban(john_rotterdam['iban']))
    ...
    family_name_similarity = validator._calculate_name_similarity(
        f"{husband['first_name']} {husband['last_name']}",
        f"{wife['first_name']} {wife['last_name']}"
    )
    
    family_email_similarity = difflib.SequenceMatcher(
        None,
        husband['email'],
        wife['email']
    ).ratio()
    
    family_iban_match = (validator._normalize_iban(husband['iban']) == 
                        validator._normalize_iban(wife['iban']))
