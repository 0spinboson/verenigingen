...
@frappe.whitelist()
def debug_coa_migration():
    """Debug CoA migration process"""
    ...
    if draft_migration:
        print(f"\n=== Found Draft Migration: {draft_migration} ===")
        doc = frappe.get_doc("E-Boekhouden Migration", draft_migration)
        return doc
    else:
        print("\n=== Creating New Test Migration ===")
        # Create a new migration for testing
        new_migration = frappe.new_doc("E-Boekhouden Migration")
        new_migration.migration_name = "Debug CoA Migration Test"
        new_migration.migration_type = "Partial Migration"
        new_migration.migrate_accounts = 1
        new_migration.migrate_cost_centers = 0
        new_migration.migrate_transactions = 0
        new_migration.dry_run = 0
        new_migration.clear_existing_accounts = 0
        
        new_migration.save()
        print(f"Created new migration: {new_migration.name}")
...
def run_coa_migration_test():
    ...
    migration = debug_coa_migration()
    
    if migration.migration_status != "Draft":
        print(f"Migration {migration.name} is not in Draft status (current: {migration.migration_status})")
        return
...
if __name__ == "__main__":
    # This won't work directly, use bench execute
    print("Use: bench --site dev.veganisme.net execute verenigingen.debug_coa_migration.function_name")