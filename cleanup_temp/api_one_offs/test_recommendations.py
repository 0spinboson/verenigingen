...
@frappe.whitelist()
def test_reservering_recommendations():
    """Test recommendations for reservering accounts"""
    frappe.set_user("Administrator")
    
    # Get reservering accounts
    ...
    """, ("Ned Ver Vegan", "%reservering%"), as_dict=True)
    ...
    # Filter for reservering accounts
    reservering_recs = []
    if result.get("success") and result.get("recommendations"):
        for rec in result["recommendations"]:
            if "reservering" in rec.get("account_name", "").lower():
                reservering_recs.append({
                    "account": rec["account"],
                    ...
                    "parent_group": rec.get("parent_group_number")
                })
    
    return {
        "accounts": accounts,
        "recommendations": reservering_recs
    }