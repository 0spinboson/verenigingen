...
def get_or_create_migration_warehouse_fixed(company):
    ...
    try:
        ...
        existing = frappe.db.get_value("Warehouse", {
            "warehouse_name": ["like", f"%e-Boekhouden Migration%"],
            "company": company
        }, "name")
...
def clean_duplicate_migration_warehouses(company):
    ...
    for warehouse in warehouses[1:]:
        try:
            ...
            has_stock = frappe.db.exists("Stock Ledger Entry", {
            ...
            frappe.log_error(
                f"Could not delete duplicate warehouse {warehouse.name}: {str(e)}",
                "Warehouse Cleanup Error"
            )
    
    return {
        "cleaned": cleaned,
        "kept": 1,
        "kept_warehouse": kept.name
    }