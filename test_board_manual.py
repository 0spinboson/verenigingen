...
def test_board_addition():
    ...
    cmd1 = [
        ...
        'frappe.db.sql',
        '--args', "SELECT name, volunteer_name, chapter_role FROM `tabChapter Board Member` WHERE parent='Antwerpen' AND is_active=1"
    ]
    
    cmd2 = [
        'bench', '--site', 'dev.veganisme.net', 'execute', 
        'frappe.db.sql',
        '--args', "SELECT member, member_name, enabled FROM `tabChapter Member` WHERE parent='Antwerpen' AND enabled=1"
    ]
